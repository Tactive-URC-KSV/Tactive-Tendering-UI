...
const useMockState = () => {
    ...
    const handleFileUpload = (event) => {
        ...
        if (files.length > 0) {
            ...
            console.log("Files uploaded:", newAttachments.map(a => a.name).join(', '));
        }
    };
...
const TFProcess = () => {
    ...
    // --- 3. Contractor Details Tab Content ---
    ...
    return (
        <div className='container-fluid min-vh-100'>
            ...
            <div className={`d-flex justify-content-${currentTab === 'boq' ? 'end' : 'between'} align-items-center mt-5 me-3 ms-3 mb-4`}>
                ...
                {/* The disabled condition below assumes an item must be selected to proceed from BOQ selection */}
                <button className="btn action-button" disabled={selectedBoq.size === 0 && currentTab === 'boq'} onClick={handleNextTabChange}><span className="fw-bold me-2">Confirm & Proceed</span><ArrowRight size={18} /></button>
            </div>
        </div> // This closes the main div
    ); // This closes the return statement
} // This closes the TFProcess component
