...
function AddResource() {
    ...
    const customStyles = {
        control: (provided, state) => ({
            ...
            minHeight: '38px', 
            // Select will now fill its wrapper div
            width: '100%',
        }),
    ...
    // --- CORRECTED FormSectionContainer for Universal Continuous Border ---
    const FormSectionContainer = ({ title, icon, defaultOpen = false, children }) => {
        ...
        const [isOpen, setIsOpen] = useState(defaultOpen); // Use state for collapsible sections

        // Styles for header in both open/closed states
        const headerStyle = {
            cursor: isStatic ? 'default' : 'pointer',
            listStyle: 'none',
            backgroundColor: containerBgColor,
            border: `1px solid ${containerBorderColor}`,
            // Border bottom is removed if it's static OR if it's open (to connect to content)
            borderBottom: (isStatic || isOpen) ? 'none' : `1px solid ${containerBorderColor}`, 
            // Rounded corners on top only if static/open, otherwise round all corners
            borderRadius: (isStatic || isOpen) ? '0.5rem 0.5rem 0 0' : '0.5rem',
            marginBottom: '0', // Keep margin at 0 here, add it to the outer div
        };

        // Styles for content, only visible if open or static
        const contentStyle = { 
            padding: '1rem 1.5rem', 
        ...
        if (isStatic) {
            // Render static section as a merged box
            return (
                <div className="mb-4">
        ...
        // Render collapsible section using a simple click handler
        return (
            <div className="mb-4">
                <div style={headerStyle} onClick={() => setIsOpen(!isOpen)}>{HeaderContent}</div>
    ...
    return (
        <div className="container-fluid min-vh-100">
            ...
            {/* --- BOQ Summary Container (Rounded Top, Square Bottom) --- */}
            <div 
                ...
                style={{ 
                    ...
                    borderRadius: '0.5rem 0.5rem 0 0', // Crucial: Ensures square bottom corners
                }} 
            >
                <div className="d-flex align-items-center">
            ...
            {/* --- Main Content Area (No Top Border) --- */}
            <div 
                ...
                style={{ 
                    ...
                    borderRadius: '0 0 0.5rem 0.5rem',
                    border: `1px solid ${containerBorderColor}`, 
                    borderTop: 'none', // Critical: Removes top border to visually connect with BOQ Summary
                    boxShadow: '0 4px 6px rgba(0,0,0,0.05)'
                }}
            >
                {/* Basic Information (Static, Continuous Border) */}
                <FormSectionContainer title="Basic Information" icon={<span className="text-primary" style={{ fontSize: '1.2em' }}>•</span>}>
                    <div className="row g-3">
                        {/* LEFT COLUMN: Resource Type (Unchanged) */}
                        ...
                        {/* LEFT COLUMN: Resource Name (Unchanged) */}
                        <div className="col-md-6">
                ...
                {/* Wastage & Net Quantity (Collapsible, Continuous Border when Open) */}
                ...
                {/* Pricing & Currency (Collapsible, Continuous Border when Open) */}
                ...
                {/* Cost Summary (Static, Continuous Border) */}
