...
// Define the primary color (matching the blue in your images)
...
function Contractor() {
    ...
    // === MODIFIED: SORT BY STATE FOR SINGLE SELECTION ===
    const [isSortDropdownOpen, setIsSortDropdownOpen] = useState(false);
    
    // State now stores the single selected value for each category (e.g., 'Verified' or null)
    const [selectedSortOptions, setSelectedSortOptions] = useState({
        status: null, // Stores 'Verified' or 'Pending' or null
        grade: null,  // Stores 'A Grade' or 'B Grade' or null
    });
    ...
    // === MODIFIED: HANDLER FOR SINGLE SELECTION (RADIO BEHAVIOR) ===
    const handleSortChange = (category, value) => {
        setSelectedSortOptions(prev => ({
            ...prev,
            [category]: prev[category] === value ? null : value, // Toggle logic: if already selected, deselect (set to null), otherwise select the new value
        }));
    };
    // === END OF MODIFIED STATE AND HANDLERS ===
    ...
    // === MODIFIED: HELPER COMPONENT FOR VISUAL MATCH AND RADIO LOGIC ===
    const CheckboxFilterItem = ({ category, label }) => {
        ...
        return (
            <div 
                ...
                // Styling to exactly match the look of the image (blue text/checkbox, light blue background on selected)
                style={{ 
                    cursor: 'pointer', 
                    paddingLeft: '1rem', // Match internal padding of dropdown items
                    paddingRight: '1rem',
            ...
            >
                <input
                    type="radio" // Changed to radio for single selection visual cue
                    name={category} // Grouped by category name for native radio behavior hint (though logic is controlled by React)
                    checked={isSelected}
                    ...
                    style={{ 
                        ...
                        transform: 'scale(1.15)' // Slightly larger radio button for better match
                    }}
                />
                <span 
                    className="fw-normal"
                    style={{ 
                        ...
                        fontSize: '0.9rem' // Adjusted font size for exact match
                    }}
                >
                    {label}
                </span>
    ...
    // === END OF MODIFIED HELPER COMPONENT ===
