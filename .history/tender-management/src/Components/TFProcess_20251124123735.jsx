...
<div className="p-4 mt-5 bg-white rounded-3" style={{ border: '1px solid #e0e0e0', boxShadow: '0 2px 4px rgba(0,0,0,.05)' }}>
    ...
    <div className="ms-1 me-1">
        {Object.keys(attachments).map((key, index) => {
            ...
            const isLast = index === Object.keys(attachments).length - 1;

            return (
                <div 
                    key={key} 
                    className={`d-flex justify-content-between align-items-center py-3 ${!isLast ? 'border-bottom' : ''}`} 
                >
                    <div className="text-start me-4 flex-grow-1">
                        
                        {/* 1. Title (Matches Figma bold/dark text) */}
                        <span className="fw-medium d-block mb-1" style={{ color: '#333' }}>{title}</span>
                        
                        {/* 2. File Type and Size (MATCHING FIGMA RED COLOR) */}
                        {fileInfo ? (
                            <div className="d-block mt-1">
                                <span style={{ fontSize: '13px', color: '#dc3545' }}> 
                                    {/* Using space separator as confirmed */}
                        ...
                        {/* 3. Description/Notes (Muted text below file details) */}
                        <span className="text-muted d-block mt-1" style={{ fontSize: '13px' }}>
                            {notes}
                        </span>
                    </div>
                    
                    {/* 4. Download Icon (Blue Circle/Light Blue Background) */}
                    {fileInfo && (
                        <span 
