...
function AddResource() {
    ...
    const FormSectionContainer = ({ title, icon, defaultOpen = false, children }) => {
        // UPDATED: 'Cost Summary' is now included here to make it static and always open.
        const isStatic = ['Basic Information', 'Quantity & Measurements', 'Cost Summary'].includes(title);
        
        // State is initialized to true for static sections.
        ...
        const HeaderContent = (
            <div className="py-3 px-4 d-flex justify-content-between align-items-center">
                ...
                {/* Chevron icon only appears on non-static sections */}
        ...
        return (
            <div className="mx-3 mb-4">
                <div 
                    style={headerStyle} 
                    // Click handler only for non-static headers
                    onClick={() => { if (!isStatic) setIsOpen(!isOpen); }} 
                >
                    {HeaderContent}
                </div>
                {/* Content is always rendered if static, or if state is open */}
