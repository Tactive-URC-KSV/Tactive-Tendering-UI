...
// Assuming these imports are available in your routing context
import { useNavigate } from "react-router-dom";
import { ArrowLeft, Pencil, Mail, UploadCloud, Calendar, FileText } from "lucide-react";

// Define the primary blue color used throughout the component
const bluePrimary = '#005197';

// Mock component to simulate a page view for creating a new contractor
function ContractorOverview() {
    ...
    const ManualEntryForm = () => (
        <form onSubmit={handleSubmit}>
            {/* MODIFIED: Applied negative horizontal margins (-1.5rem) to counteract 
              the parent card's 'p-4' padding. Also adjusted border radius to 
              ensure the top corners match the card, and the bottom corners are square. 
            */}
            <div
                className="p-3 mb-4 d-flex align-items-center" // Removed 'rounded' class
                style={{
                    backgroundColor: bluePrimary,
                    // Use negative margin to span the full width of the card
                    marginLeft: '-1.5rem',
                    marginRight: '-1.5rem',
                    // Match the parent card's top radius (8px)
                    borderTopLeftRadius: '8px',
                    borderTopRightRadius: '8px',
                    // Remove bottom radius for flush look with form content
                    borderBottomLeftRadius: '0',
                    borderBottomRightRadius: '0',
                    paddingLeft: '1.5rem', // Restore the visual padding inside the blue bar
                    paddingRight: '1.5rem',
                }}
            >
    ...
    return (
        <div className="container-fluid min-vh-100 p-0">
            ...
            <div className="px-4 text-start">
                ...
                {/* Content Card */}
                {/* The p-4 class here is what the header is fighting against */}
                <div className="card p-4 text-start border-0 shadow-sm" style={{ borderRadius: '8px' }}>
                    {selectedView === 'manual' ? (
                        <ManualEntryForm />
                    ) : (
                        <div>
