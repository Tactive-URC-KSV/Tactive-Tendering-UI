...
function ContractorReview() {
    ...
    // Check if data from location.state is empty (which happens on refresh or truncation)
    // If empty, retrieve the complete, full data from Session Storage
    if (Object.keys(formData).length <= 2) { // Checking if it's mostly empty (e.g., just key/message)
        try {
            const savedData = sessionStorage.getItem(STORAGE_KEY);
            if (savedData) {
                formData = JSON.parse(savedData);
                console.warn("Loaded form data from sessionStorage as location.state was empty or incomplete.");
            }
        } catch (error) {
            console.error("Error loading form data from sessionStorage:", error);
        }

    // 🌟 THIS FUNCTION RELIES ON 'fileData' BEING A BASE64 STRING
                            ...
                            {hasAttachments ? (
                                attachmentMetadata.map((file, index) => (
                                    ...
                                    >
                                        ...
                                        {/* 🌟 THIS IS THE CORRECT VIEW BUTTON CALL */}
                                        <button 
