...
<div className="p-4 mt-5 bg-white rounded-3" style={{ border: '1px solid #e0e0e0', boxShadow: '0 2px 4px rgba(0,0,0,.05)' }}>
    ...
    {/* Content Area - REMOVED THE INNER BORDERED DIV HERE */}
    <div className="ms-1 me-1"> {/* This div now only handles padding/margin, no border */}
        {Object.keys(attachments).map((key, index) => {
            ...
            const isLast = index === Object.keys(attachments).length - 1; // Check if it's the last item

            return (
                <div 
                    key={key} 
                    // Apply border-bottom to all items *except* the last one
