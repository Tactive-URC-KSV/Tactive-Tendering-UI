...
function ContractorOverview() {
    ...
    const ManualEntryForm = () => (
        <>
            ...
            <>
    <div className="p-3 mb-4 d-flex align-items-center justify-content-center" style={{ backgroundColor: '#005197', width: "calc(100% + 3rem)", marginLeft: "-1.5rem", marginRight: "-1.5rem", borderRadius: "8px 8px 0 0" }}>
        <FileText size={20} className="me-2 text-white" />
        <h3 className="mb-0 fs-6 fw-bold text-white">Basic Information</h3>
    </div>
    <div className="row mb-3">{/* ... entityCode & entityName ... */}</div>
    <div className="row mb-3">
        <div className="col-md-6 mb-3">
            <label htmlFor="effectiveDate" className="projectform text-start d-block">Effective Date</label>
            {/* DatePicker component insertion here */}
        </div>
        <div className="col-md-6 mb-3">
            <label htmlFor="entityType" className="projectform-select text-start d-block">Entity Type</label>
            {/* Select component insertion here */}
        </div>
    </div>
    <div className="row mb-4">
        <div className="col-md-6 mb-3">{/* ... natureOfBusiness ... */}</div>
        <div className="col-md-6 mb-3">{/* ... grade ... */}</div>
    </div>
    
    {/* --- EXACT ATTACHMENT SECTION FROM IMAGE --- */}
    <h4 className="fs-6 fw-bold mb-3">Attachment (Certificates/Licenses)</h4>
    <div 
        className="border border-dashed p-4 text-center rounded" 
        style={{ 
            cursor: "pointer", 
            // Matching the dotted line and border color
            border: "2px dashed #a0c3e3", 
        }} 
        // NOTE: Uses fileInputRef and onClick to make the whole box clickable
    ...
    >
        <input 
            ...
            multiple
        />
        <label htmlFor="fileUpload" className="d-block cursor-pointer p-3 m-0">
            {/* Lucide UploadCloud icon with color and size matching image */}
            <UploadCloud size={48} className="mb-2 mx-auto" style={{ color: '#6c96b7' }} />
            {/* Primary text matching color and style */}
            <p className="mb-1" style={{ color: '#005197', fontWeight: 500 }}>Click to upload or drag and drop</p>
            {/* Secondary text matching file type/size limits */}
            <p className="mb-0 small text-muted">PDF, DOCX up to 10MB</p>
        </label>
    </div>
    {/* --- END OF EXACT ATTACHMENT SECTION --- */}

    {/* Display uploaded files (using your original display logic) */}
    {uploadedFiles.length > 0 && (
        <div className="mt-3 text-start">
            <h5 className="fs-6 fw-bold mt-3 mb-2">Uploaded Files:</h5>
            <div className="row g-3">
                {uploadedFiles.map((file, idx) => (
                    <div key={idx} className="col-12 col-sm-6 col-md-4 col-lg-3">
                        <div className="d-flex align-items-center justify-content-between border rounded p-2 bg-light">
                            <div className="small text-truncate me-2" title={file.name}>{file.name}</div>
                            <X
                                size={16}
