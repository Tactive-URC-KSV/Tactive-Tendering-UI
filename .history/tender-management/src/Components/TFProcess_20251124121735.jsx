...
<div className="p-4 mt-5 bg-white rounded-3" style={{ border: '1px solid #e0e0e0', boxShadow: '0 2px 4px rgba(0,0,0,.05)' }}>
    ...
    <div className="ms-1 me-1 p-3" style={{ border: '1px solid #eee', borderRadius: '8px' }}>
        {Object.keys(attachments).map((key) => {
            ...
            // Assume the first file holds the data (type, size, etc.)
            const fileInfo = files.length > 0 ? files[0] : null;

            // Helper functions (defined outside the component) are still used here:
            // getFriendlyFileType(fileInfo.type || fileInfo.extension) 
            ...
            return (
                ...
                >
                    <div className="text-start me-4 flex-grow-1">
                        ...
                        {/* 2. File Type and Size (NO RED COLOR, using | separator) */}
                        {fileInfo ? (
