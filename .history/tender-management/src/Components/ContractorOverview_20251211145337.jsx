...
const [formData, setFormData] = useState(loadFormData());
  ...
  const handleFileChange = async (event) => { // <<< MUST BE ASYNC
    ...
    // Loop through files to convert each one
    for (const file of files) {
        try {
            // 1. AWAIT the asynchronous conversion
            const fileDataUrl = await fileToBase64(file);
            
            // 2. Store metadata PLUS the Base64 data
            newAttachments.push({
                name: file.name,
                size: file.size,
                lastModified: file.lastModified,
                fileData: fileDataUrl // <-- This 'fileData' is essential for viewing
            });
        } catch (error) {
            console.error("Error converting file to Base64:", error);
            // Optionally, handle error (e.g., skip the file or show a message)
        }
    }
    
    // Update the form state with the newly processed attachments
    setFormData(prevData => {
        ...
        // Save updated data to Session Storage for persistence across pages
