...
function ContractorOverview() {
   ...
   // --- ✅ CODE TO ADD (New State Initialization and Persistence) ✅ ---

   // --- ✅ CODE TO ADD (New State Initialization and Persistence) ✅ ---

// 1. Define the complete default state structure (used if no saved data exists)
const defaultFormData = {
    ...
    contractorName:'', 
    contractorMessage:'',
    attachmentMetadata: [] // <<< NEW: For persistent display of filenames
};

// 2. Function to load data from Session Storage
const loadFormData = () => {
    try {
        const savedData = sessionStorage.getItem(STORAGE_KEY);
        if (savedData) {
            const parsedData = JSON.parse(savedData);
            // Load saved metadata, but reset volatile attachments (files are non-serializable)
            return { 
                ...defaultFormData, 
                ...parsedData, 
                attachments: [], 
                attachmentMetadata: parsedData.attachmentMetadata || [] // <<< NEW: Load saved metadata
            };
        }
    } catch (error) {
...
// 3. Initialize state using the loading function
const [formData, setFormData] = useState(loadFormData());

// 4. Hook to save state to session storage every time formData changes
useEffect(() => {
    // Exclude the non-serializable 'attachments' before saving
    const serializableData = { 
        ...formData, 
        attachments: [], 
        attachmentMetadata: formData.attachmentMetadata // <<< NEW: Save persistent metadata
    }; 
    sessionStorage.setItem(STORAGE_KEY, JSON.stringify(serializableData));
}, [formData]);

// --- The rest of your function (handleChange, handleSelectChange, handleSubmit, etc.) remains unchanged. ---
...
// --- The rest of your function (handleChange, handleSelectChange, handleSubmit, etc.) remains unchanged. ---
   ...
   const handleFileChange = (e) => {
    ...
    // <<< NEW: Create JSON-safe metadata for display/persistence
    ...
    setFormData(prev => ({ 
        ...prev, 
        attachments: [...prev.attachments, ...newFiles], // Keeps actual file object
        attachmentMetadata: [...prev.attachmentMetadata, ...newMetadata] // <<< NEW: Keeps metadata for persistence
    }));
};
    ...
    setFormData(prev => ({
        ...
        // <<< NEW: Filter the metadata array alongside the file array
