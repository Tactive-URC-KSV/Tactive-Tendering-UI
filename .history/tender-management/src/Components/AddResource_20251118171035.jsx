...
const fetchResources = useCallback((resTypeId) => {
  if (!resTypeId) return; // Don't fetch if no Resource Type selected
...
const resourceOptions = useMemo(
  () =>
    resources.length > 0
      ? resources.map((item) => ({ value: item.id, label: item.resourceName }))
      : [{ value: '', label: 'No resources available' }],
  [resources]
);
  ...
  <div style={{ width: '80%' }}>
    <Select
      options={resourceOptions}  // <-- use the options fetched from API
      styles={{
        ...customStyles,
        placeholder: (provided) => ({
          ...provided,
      ...
      onChange={(selected) => {
        // optionally store the selected resource in state
