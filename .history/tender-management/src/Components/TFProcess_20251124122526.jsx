...
const getFriendlyFileType = (mimeType) => {
    ...
    if (mimeType.includes('image/')) {
        return mimeType.split('/')[1].toUpperCase() || 'Image'; // e.g., JPEG, PNG
    }
...
<div className="p-4 mt-5 bg-white rounded-3" style={{ border: '1px solid #e0e0e0', boxShadow: '0 2px 4px rgba(0,0,0,.05)' }}>
    ...
    <div className="table-responsive ms-1 me-1">
      ...
      {/* 5. ATTACHMENTS - SINGLE WHITE CONTAINER CARD (Outer structure) */}
<div className="p-4 mt-5 bg-white rounded-3" style={{ border: '1px solid #e0e0e0', boxShadow: '0 2px 4px rgba(0,0,0,.05)' }}>
    
    {/* Header for Attachments Section */}
    <div className="text-start ms-1 mb-3">
        {/* Ensure Paperclip is imported */}
        <Paperclip size={20} color="#005197" /> 
        <span className="ms-2 fw-bold" style={{ color: '#005197' }}>Attachments</span>
    </div>

    {/* Content Area (Inner Gray Bordered Box) - This has the light gray border color #eee */}
    <div className="ms-1 me-1 p-3" style={{ border: '1px solid #eee', borderRadius: '8px' }}>
        {Object.keys(attachments).map((key) => {
            ...
            // Helper functions (getFriendlyFileType, formatFileSize) are required and must be defined outside the component.

            return (
                <div 
                    key={key} 
                    // Individual rows use a light gray border at the bottom for separation
                ...
                >
                    <div className="text-start me-4 flex-grow-1">
                        ...
                        {/* File Type and Size (Text Color: #6c757d - Muted Gray) */}
                        {fileInfo ? (
