    ...
    <>
        ...
        <div className="row mb-3">
            <div className="col-md-6 mt-3 mb-4">
                ...
                <input
    type="text"
    name="entityCode" // IMPORTANT: Add the name attribute for scrolling
    className={`form-input w-100 ${errors.entityCode ? 'is-invalid-field' : ''}`} // ADD dynamic class
    placeholder="Enter Entity Code"
    value={formData.entityCode}
    onChange={(e) => setFormData({ ...formData, entityCode: e.target.value })}
/>
{errors.entityCode && <p className="text-danger small mt-1">{errors.entityCode}</p>} // ADD error message
            </div>
    ...
    className={errors.entityType ? 'is-invalid-field-select' : ''} // ADD dynamic class to main wrapper
    value={entityTypeOptions.find(option => option.value === formData.entityType)}
    placeholder="Select entity type"
/>
{errors.entityType && <p className="text-danger small mt-1">{errors.entityType}</p>} // ADD error message
            </div>
        </div>
 ...
 const AddressDetailsContent = ({ formData, setFormData, handleSelectChange, errors }) => (
...
function ContractorOverview() {
    ...
    // Define MANDATORY fields for manual entry
    ...
    // Define MANDATORY fields for email invite mode
...
const [formData, setFormData] = useState(loadFormData());
    ...
    const handleSelectChange = (selectedOption, actionMeta) => {
        ...
        // Clear error when select option is chosen
        setErrors(prev => ({ ...prev, [name]: '' }));
    };
...
const handleSubmit = (e) => {
        ...
        if (validateForm()) {
            ...
            if (firstErrorField) {
                // Find and scroll to the first element with the name of the error field
    ...
    const handleSendInvitation = (e) => {
        ...
        if (validateForm()) {
            ...
            // Logic to send email/show success message
        }
    };
