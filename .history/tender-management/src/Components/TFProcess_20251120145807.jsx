...
useEffect(() => {
  if (Array.isArray(tenderDetail?.scopeOfPackage) && tenderDetail.scopeOfPackage.length > 0) {
    ...
    control: (provided, state) => ({
      ...provided,
      marginTop: '6px',        // <- pushes the select down so border won't overlap the label
      minHeight: '44px',      // <- matches typical input height
      boxShadow: state.isFocused ? '0 0 0 0.15rem rgba(0,81,151,0.12)' : provided.boxShadow
    }),
    placeholder: (prov) => ({ ...prov, color: '#6c757d' }),
  }}
    />
</div>
