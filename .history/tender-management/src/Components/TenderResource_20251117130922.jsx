...
<div className="btn-group me-2" role="group" aria-label="View switch">
            {/* Table View Button (Left side) */}
            <button
                type="button"
                className="btn btn-sm text-center"
                onClick={() => setViewType('table')}
                style={{
                    backgroundColor: viewType === 'table' ? APP_BLUE : 'white',
                    color: viewType === 'table' ? 'white' : APP_BLUE,
                    borderColor: APP_BLUE, // Use the blue color for the border
                    borderRight: viewType === 'grid' ? `1px solid ${APP_BLUE}` : 'none',
                    borderRadius: '5px 0 0 5px',
                    padding: '8px 12px'
                }}
                title="Table View"
            >
                <Table size={20} color={viewType === 'table' ? 'white' : APP_BLUE} />
            </button>
            
            {/* Grid View Button (Right side) */}
            <button
                type="button"
                className="btn btn-sm text-center"
                onClick={() => setViewType('grid')}
                style={{
                    backgroundColor: viewType === 'grid' ? APP_BLUE : 'white',
                    color: viewType === 'grid' ? 'white' : APP_BLUE,
                    borderColor: APP_BLUE, // Use the blue color for the border
                    borderLeft: viewType === 'table' ? `1px solid ${APP_BLUE}` : 'none',
                    borderRadius: '0 5px 5px 0',
                    padding: '8px 12px'
                }}
                title="Grid View"
            >
                <Grid size={20} color={viewType === 'grid' ? 'white' : APP_BLUE} />
            </button>
        </div>
          </div>
        </div>
