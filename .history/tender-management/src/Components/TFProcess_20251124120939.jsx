...
<div className="mt-5">
    ...
    {Object.keys(attachments).map((key) => {
        ...
        // Assuming files[0] holds the file information (type, size, extension)
        const fileInfo = files.length > 0 ? files[0] : null;

        // Helper function to format the file size (You should define this outside the component)
                        ...
                        {fileInfo ? (
                            <div className="d-block mt-1">
                                <span className="fw-medium me-2" style={{ fontSize: '13px', color: '#dc3545' }}>
                                    {/* Displaying File Type | Formatted Size */}
                                    {/* Using fileInfo.type (MIME type) or fileInfo.extension (like .pdf) */}
                                    {fileInfo.type || fileInfo.extension || 'File'} | {formatFileSize(fileInfo.size) || fileInfo.id || 'N/A'}
                                </span>
                            </div>
                        ) : (
