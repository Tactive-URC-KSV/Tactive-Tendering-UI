...
const ReviewField = ({ label, value }) => (
    // Uses col-md-6 for two columns per row, mt-3/mb-3 for consistent vertical padding.
    <div className="col-md-6 mt-3 mb-3"> 
        {/* Label: Small, light grey, tight bottom margin */}
        <label className="text-start d-block mb-1" style={{ fontSize: '0.75rem', color: '#6c757d' }}>
            {label}
        </label>
        {/* Value: Bold, dark text, standard size */}
        <p className="mb-0 fw-bold text-dark" style={{ fontSize: '0.85rem' }}> 
            {/* If value is null/undefined/empty string, display the literal 'N/A' string */}
            {value || 'N/A'} 
        </p>
    </div>
);
...
const ReviewSectionWrapper = ({ title, icon: Icon, children }) => (
    <div className="card text-start border-0 shadow-sm mt-4" style={{ borderRadius: "8px", padding: "0 1.5rem 1.5rem 1.5rem" }}>
        {/* Blue Header Bar styling to match the full-width header in your design */}
...
function ContractorReview() {
    ...
    // Safely attempt to read the 'formData' from the state
    ...
    return (
        <div className="container-fluid min-vh-100 p-0">
            {/* Top Navigation Header (Back button and title) */}
