...
const [formData, setFormData] = useState(loadFormData());
  ...
  const handleFileChange = async (event) => { // <<< Must be ASYNC
    const files = Array.from(event.target.files);
    const newAttachments = [];

    for (const file of files) {
        try {
            // AWAIT the conversion to ensure you get the full, non-corrupted Base64 string
            const fileDataUrl = await fileToBase64(file);
            
            // Log the length to verify data conversion happened correctly (for debugging)
            console.log(`[Base64 Conversion Success] File: ${file.name}, Base64 Length: ${fileDataUrl.length}`);

            newAttachments.push({
                name: file.name,
                size: file.size,
                lastModified: file.lastModified,
                fileData: fileDataUrl // <<< CRITICAL: Saves the Base64 content
            });
        } catch (error) {
    ...
    // Recommended: Clear the file input value so the same file can be selected again
