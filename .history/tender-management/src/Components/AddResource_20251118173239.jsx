...
function AddResource() {
    ...
    const [uomOption, setUomOption] = useState([]); // UOM state added
    ...
    const fetchResourceTypes = useCallback(() => {
        ...
        .catch(err => { if(err?.response?.status === 401) handleUnauthorized(); else toast.error('Failed to fetch resource types.'); });
    }, []);
    ...
    const fetchResourceNature = useCallback(() => {
        ...
        .catch(err => { if(err?.response?.status === 401) handleUnauthorized(); else toast.error('Failed to fetch resource natures.'); });
    }, []);
    ...
    const fetchResources = useCallback((resTypeId) => {
        ...
        .catch(err => { if(err?.response?.status === 401) handleUnauthorized(); else toast.error('Failed to fetch resources.'); });
    }, []);
    ...
    const fetchQuantityType = useCallback(() => {
        ...
        .catch(err => { if(err?.response?.status === 401) handleUnauthorized(); else toast.error('Failed to fetch quantity types.'); });
    }, []);
    ...
    const fetchCurrency = useCallback(() => {
        ...
        .catch(err => { if(err?.response?.status === 401) handleUnauthorized(); else toast.error('Failed to fetch currencies.'); });
    }, []);
    ...
    const fetchUOM = useCallback(() => {
        ...
        .then(res => {
            if(res.status === 200) {
                setUomOption(res.data.map(u => ({ value: u.id, label: u.uomName })));
            }
        })
        .catch(err => { if(err?.response?.status === 401) handleUnauthorized(); else toast.error("Failed to fetch UOMs."); });
    }, []);
    ...
    return (
        <div className="container-fluid min-vh-100">
            ...
            <div className="text-white p-3 d-flex justify-content-between align-items-center mx-3 mb-4" style={{ background: `linear-gradient(to right, ${darkBlue}, ${vibrantBlue})`, borderRadius: '0.5rem' }}>
            ...
            <FormSectionContainer title="Basic Information" icon={<span className="text-primary" style={{ fontSize: '1.2em' }}>•</span>} defaultOpen={true}>
                <div className="row g-3">
                    <div className="col-md-6">
                        ...
                        <div style={{ width: '80%' }}>
                            <Select options={resourceTypeOptions} styles={customStyles} placeholder="Select Resource Type"
                    ...
                    <div className="col-md-6">
                        ...
                        <div style={{ width: '80%' }}>
                            <Select options={resourceOption} styles={{...customStyles, placeholder: (p)=>({...p,color:'black',textAlign:'left'}), singleValue: (p)=>({...p,color:'black'})}} placeholder="Select resource"/>
                        </div>
                    </div>
            ...
            <FormSectionContainer title="Quantity & Measurements" icon={<AlignLeft size={20} className="text-primary" />} defaultOpen={true}>
                <div className="row g-3">
                    <div className="col-md-6">
                        ...
                        <div style={{ width: '80%' }}>
                            <Select options={uomOption} styles={customStyles} placeholder="Select UOM"/>
                        </div>
                    </div>
            ...
            {/* The rest of your sections (Wastage & Net Quantity, Pricing & Currency, Cost Summary) remain unchanged */}
