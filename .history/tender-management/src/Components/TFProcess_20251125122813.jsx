    ...
    <div className="d-flex flex-wrap gap-2 mt-1">
        {Array.isArray(tenderDetail.scopeOfPackage) && tenderDetail.scopeOfPackage.length > 0 ? (
            tenderDetail.scopeOfPackage.map((scopeId, index) => {
                ...
                return (
                    // 🚨 USE THE react-select CLASSES FOR CONSISTENCY 🚨
                    <span key={index} className="select__multi-value">
                        <span className="select__multi-value__label">
                            {scopeObj ? scopeObj.label : scopeId}
                        </span>
                        {/* 🚨 ADD THE FUNCTIONAL REMOVE BUTTON 🚨 */}
                        <span 
                            className="select__multi-value__remove"
                            onClick={(e) => {
                                e.preventDefault();
                                // You must define this function to update tenderDetail.scopeOfPackage
                                // Example: handleRemoveTenderScope(scopeId); 
                            }}
                        >
                            &times;
                        </span>
