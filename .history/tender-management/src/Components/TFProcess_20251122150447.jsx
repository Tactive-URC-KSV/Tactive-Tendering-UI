...
const handleNoteChange = (key, value) => {
    ...
    const attachmentCount = Object.keys(attachments).filter(key => 
        attachments[key].fileName || attachments[key].fileSize || attachments[key].notes
    ).length;
const floatingDate = new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
    const tenderFloatingNo = 'TF - XXXX - 001'; // Placeholder
    const currentContractors = contractorsList;
  return (
        <div className="p-4">
            ...
            <div className="d-flex justify-content-start gap-5 ms-1" style={{ borderBottom: '1px solid #00519933', paddingBottom: '15px' }}>
                <div>
                    ...
                    <span className="fw-bold" style={{ color: '#005197', fontSize: '16px' }}>{tenderFloatingNo}</span>
                </div>
                <div>
                    <span className="text-muted d-block" style={{ fontSize: '14px' }}>Floating Date</span>
                    <span className="fw-bold" style={{ color: '#333', fontSize: '16px' }}>{floatingDate}</span>
                </div>
            </div>
            ...
            <div className="pt-4">
                ...
                <div className="row g-4 ms-1" style={{ fontSize: '15px' }}>
                    <div className="col-md-4 text-start">
                        ...
                        <span className="fw-medium">{tenderDetail.projectName || 'N/A'}</span>
                    </div>
                    <div className="col-md-4 text-start">
                        <span className="text-muted d-block">Offer Submission Mode</span>
                        <span className="fw-medium">{tenderDetail.offerSubmissionMode || 'N/A'}</span>
                    </div>
                    <div className="col-md-4 text-start">
                        <span className="text-muted d-block">Submission Last Date</span>
                        {/* Assuming tenderDetail.offerSubmissionDate holds the value */}
                        <span className="fw-medium text-danger">{tenderDetail.offerSubmissionDate || 'N/A'}</span> 
                    </div>
                    <div className="col-md-4 text-start">
                    ...
                    <div className="col-md-12 text-start">
                        <span className="text-muted d-block mb-1">Scope of Packages</span>
                        <div className="d-flex flex-wrap gap-2">
                            {/* Assuming selectedScopes is stored in tenderDetail */}
                            {tenderDetail.selectedScopes?.length > 0 ? (
                                tenderDetail.selectedScopes.map((scope, index) => (
                                    <span key={index} className="badge p-2" style={{ backgroundColor: '#E0F0E0', color: '#2BA95A', fontSize: '12px', fontWeight: 'normal' }}>
                                        {scope}
                                    </span>
                                ))
                            ) : (
                                <span className="text-muted" style={{ fontSize: '14px' }}>N/A</span>
                            )}
                        </div>
                    </div>
                </div>
                ...
                <div className="text-start ms-1 mt-5 mb-3">
                    ...
                    <span className="ms-2 fw-bold" style={{ color: '#005197' }}>Package Details ({boqArray.length})</span>
                </div>
                <div className="table-responsive ms-1 me-1">
                    <table className="table table-borderless table-sm">
                        <thead style={{ color: '#005197' }}>
                            <tr>
                                <th style={{ width: '80px' }}>BOQ Code</th>
                                <th>BOQ Name</th>
                                <th style={{ width: '80px' }}>Unit</th>
                                <th className="text-end" style={{ width: '100px' }}>Quantity</th>
                            </tr>
                        </thead>
                        <tbody>
                            {boqArray.length > 0 ? (
                                ...
                                <tr><td colSpan="4" className="text-center text-muted py-4">No BOQ items selected for this package.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
                ...
                <div className="text-start ms-1 mt-5 mb-3">
                    ...
                    <p className="text-muted mt-1" style={{ fontSize: '14px' }}>
                        {currentContractors.length} contractors selected for tender invitation
                    </p>
                </div>
                <div className="d-flex flex-wrap gap-3 ms-1">
                    {/* Render contractors. This will be empty as contractorsList is [] */}
                    {currentContractors.length > 0 ? (
                        ...
                        <div className="text-muted p-2" style={{ border: '1px solid #ddd', borderRadius: '8px', fontSize: '14px' }}>
                            No contractors selected for invitation.
                        </div>
                    )}
                </div>
                ...
                <div className="ms-1 me-1 p-3" style={{ border: '1px solid #eee', borderRadius: '8px' }}>
                    {/* Iterate over attachments object to render rows */}
                    {Object.keys(attachments).map((key) => (
                        <AttachmentRow 
                            key={key}
                            title={getAttachmentTitle(key)} 
                            fileName={attachments[key].files?.[0]?.name || attachments[key].fileName} // Use first file name or placeholder
                            fileSize={attachments[key].fileSize} // Assuming you store file size
                            notes={attachments[key].notes} 
                        />
                    ))}
                </div>

                {/* ----------------------------- Tender Summary & Float Button ----------------------------- */}
                <div className="p-3 mt-5 mb-5 d-flex justify-content-between align-items-center" style={{ backgroundColor: '#F0F8FF', border: '1px solid #B0C4DE', borderRadius: '8px' }}>
                    <div className="d-flex gap-5 fw-bold" style={{ color: '#005197', fontSize: '15px' }}>
                        ...
                        <div>Selected Contractors: <span style={{ color: '#333' }}>{currentContractors.length}</span></div> 
                        <div>Attachments: <span style={{ color: '#333' }}>{attachmentCount}</span></div>
                        <div>Submission Deadline: <span style={{ color: '#dc3545' }}>{tenderDetail.offerSubmissionDate || 'Not Set'}</span></div> 
                    </div>
                    <button 
                        className="btn fw-bold" 
                        style={{ backgroundColor: '#005197', color: 'white', padding: '10px 20px' }}
...
const AttachmentRow = ({ title, files, notes }) => (
  ...
  const reviewTender = () => {
        ...
        // Get the list of selected BOQs from the tree structure
        const boqArray = getSelectedLeafBoqs(parentTree, selectedBoq);
        // Use the placeholder list for contractors until you define the real state
        ...
        const getAttachmentTitle = (key) => {
            // Converts key names to the descriptive titles shown in the screenshot
        ...
        return (
            <div className="p-4">
                ...
                {/* Floating Details - Placeholders for values generated upon saving/floating */}
                <div className="d-flex justify-content-start gap-5 ms-1" style={{ borderBottom: '1px solid #00519933', paddingBottom: '15px' }}>
                    <div>
                        <span className="text-muted d-block" style={{ fontSize: '14px' }}>Tender Floating No</span>
                        <span className="fw-bold" style={{ color: '#005197', fontSize: '16px' }}>TF - {project?.projectCode || 'XXXX'} - 001</span> {/* Placeholder based on project code */}
                    </div>
                    <div>
                        <span className="text-muted d-block" style={{ fontSize: '14px' }}>Floating Date</span>
                        <span className="fw-bold" style={{ color: '#333', fontSize: '16px' }}>{new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</span> {/* Current Date as placeholder */}
                    </div>
                </div>
                ...
                <div className="pt-4">
                    ...
                    <div className="row g-4 ms-1">
                        ...
                        <div className="col-md-4 text-start">
                            <span className="text-muted d-block">Submission Last Date</span>
                            {/* Uses red color style if date is set */}
                            <span className="fw-medium" style={{color: tenderDetail.offerSubmissionDate ? '#dc3545' : '#333'}}>{tenderDetail.offerSubmissionDate || 'N/A'}</span>
                        </div>
                        <div className="col-md-4 text-start">
                            <span className="text-muted d-block">Bid Opening</span>
                            <span className="fw-medium">{tenderDetail.bidOpeningDate || 'N/A'}</span>
...
<div className="ms-1 me-1 p-3" style={{ border: '1px solid #eee', borderRadius: '8px' }}>
    {/* This maps over the keys (technical, drawings, commercial, others) in the 'attachments' state */}
    {Object.keys(attachments).map((key) => (
        <AttachmentRow 
            key={key}
            title={getAttachmentTitle(key)} 
            files={attachments[key].files || []} // This line reads the files array
            notes={attachments[key].notes || ''} // This line reads the notes string
        />
    ))}
</div>
