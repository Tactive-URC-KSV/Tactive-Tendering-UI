...
function TFProcess({ projectId }) {
    ...
    const [boqForRemoval, setBoqForRemoval] = useState(new Set()); // New state for Package Details removal selection
    ...
    // Helper function to navigate on unauthorized access (assuming `Maps` comes from `react-router-dom` or similar)
    const handleUnauthorized = () => {
        // Since 'navigate' is not defined in the provided scope, we'll log an error or assume it's available globally/via context if needed.
        console.error("Unauthorized access, attempting to redirect to login...");
        // navigate('/login'); // Assuming this function exists elsewhere in the real app
    }
    ...
    const fetchParentBoqData = async () => {
        try {
            ...
            if (err?.response?.status === 401) {
                // navigate('/login'); // Assuming navigation function exists
            }
            setParentBoq([]);
        }
    };
    ...
    // Helper function to extract only the SELECTED leaf BOQs from the tree
        ...
        const traverse = (nodes) => {
            nodes.forEach(boq => {
                if (boq.lastLevel === true && selectedBoq.has(boq.id)) {
                    // Check if it's a leaf and is selected
                    result.push(boq);
                } else if (Array.isArray(boq.children)) {
                    // If it's a parent, continue traversing
    ...
    // Functions for Package Details removal selection
    ...
    const handleRemoveSelectedBoqs = () => {
        ...
        setBoqForRemoval(new Set()); // Clear removal selection after removal
    };
            ...
            <div className="p-2">
                <div className="text-start ms-1 mt-4 d-flex justify-content-between align-items-center">
                    ...
                    <div className="d-flex gap-2">
                        ...
                        >
                            <span className="fw-medium">Remove ({boqForRemoval.size})</span>
                        </button>
                        <button
                            className="btn btn-sm"
    ...
    const tenderDetails = () => {
        // Updated call to use the new helper function
