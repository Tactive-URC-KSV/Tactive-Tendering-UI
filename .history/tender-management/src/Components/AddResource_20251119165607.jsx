...
function AddResource() {
    ...
    // Find the full resource object by ID
    const res = resources.find(r => r.id === selected.value);
    if (res) {
        // Update rate and UOM automatically
        handleCalculations({
            resourceId: res.id,
            rate: res.rate || 0,
            uomId: res.uomId
        ...
        // Update UOM select visually
        setSelectedUom({ value: res.uomId, label: res.uomName });
    }
}}
                            />
                        </div>
