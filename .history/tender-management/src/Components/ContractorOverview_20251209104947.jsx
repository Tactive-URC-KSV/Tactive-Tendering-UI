...
// Assume these imports and state/handler definitions are available in the parent component:
// import { UploadCloud, X } from 'lucide-react'; 
// const bluePrimary = '#005197';
// const [formData, setFormData] = useState({ attachments: [] }); // Updated state

/* --- Handlers to be defined in the Parent Component --- */

const handleFileChange = (e) => {
    const files = Array.from(e.target.files);
    
    // Resetting the file input value prevents React from skipping 
    // the onChange event if the same file is selected twice.
    e.target.value = null; 

    setFormData(prev => ({ 
        ...prev, 
        // Append new files to the existing array of attachments
...
<div className="row mb-4">
    <div className="col-12 mt-3">
        ...
        >
            {/* Note: Added 'multiple' attribute to allow selecting multiple files */}
            <input 
                type="file" 
                id="fileUpload" 
                name="attachments" 
            ...
            <label htmlFor="fileUpload" className="d-block cursor-pointer">
                {/* Replaced UploadCloud icon with a generic cloud for better analogy with image */}
        ...
        {/* Displaying Uploaded Files (Matching the image format) */}
        <div className="d-flex flex-wrap gap-3">
            {formData.attachments && formData.attachments.map((file, index) => (
                <div 
                    ...
                    style={{ 
                        backgroundColor: '#F7F7F7', // Light background color for the file tag
                        maxWidth: '200px', 
                        overflow: 'hidden',
                        whiteSpace: 'nowrap',
                    }}
                >
                    ...
                    >
                        {/* Custom 'X' icon if using Lucide or Fa icons */}
