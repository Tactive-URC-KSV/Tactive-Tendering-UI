...
// Import icons used in your header/design
import { ArrowLeft, CheckCircle, FileText, MapPin, Phone, Briefcase, Landmark, Info } from 'lucide-react';

// Define the primary color used in headers (assuming it's available or use the hex from your design)
const bluePrimary = "#005197"; 

/**
 * Helper component to display a single form field in the desired read-only review format.
 * Uses inline styles for critical formatting to guarantee the look from the Figma design.
 */
const ReviewField = ({ label, value }) => (
    // col-md-6 for side-by-side, mt-3/mb-3 for vertical spacing
    <div className="col-md-6 mt-3 mb-3"> 
        {/* Label style: Light grey, smaller font (matching the text above the data in the design) */}
        <label className="text-start d-block mb-1" style={{ fontSize: '0.8rem', color: '#6c757d' }}>
            {label}
        </label>
        {/* Value style: Bold, dark text, standard size */}
...
/**
 * Reusable wrapper component for each section (matches the card and blue header structure).
 */
const ReviewSectionWrapper = ({ title, icon: Icon, children }) => (
    // Card with shadow, margin, and padding adjustment
    <div className="card text-start border-0 shadow-sm mt-4" style={{ borderRadius: "8px", padding: "0 1.5rem 1.5rem 1.5rem" }}>
        {/* Blue Header Bar (p-3 mb-4 d-flex) */}
...
function ContractorReview() {
    // 1. Retrieve data passed via navigation state (THIS IS CRITICAL)
                    ...
                    <div className="col-12 mt-3 mb-3">
                        ...
                        <div className="d-flex flex-wrap mt-2">
                            {formData.attachments && formData.attachments.length > 0 ? (
                                ...
                                <p className="mb-0 fw-bold text-dark" style={{ fontSize: '0.9rem' }}>N/A</p>
                            )}
                        </div>
                    </div>
