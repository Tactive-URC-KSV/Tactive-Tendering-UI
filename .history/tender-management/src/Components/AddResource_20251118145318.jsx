...
function AddResource() {
    ...
    const FormSectionContainer = ({ title, icon, defaultOpen = false, children }) => {
        ...
        // Using local state for collapsible sections based on the previous solution
        const [isOpen, setIsOpen] = useState(defaultOpen); 
        
        // --- Header Style (Ensures top borders/rounding, and removes bottom border if open/static) ---
        const headerStyle = {
            ...
            // Rounded corners on top only if static/open, otherwise round all corners (when closed)
            borderRadius: (isStatic || isOpen) ? '0.5rem 0.5rem 0 0' : '0.5rem',
            marginBottom: '0', 
        };

        // --- Content Style (Ensures side/bottom borders, and bottom rounding) ---
        const contentStyle = { 
            padding: '1rem 1.5rem', 
        ...
        if (isStatic) {
            // Static sections are always open and merged
            return (
                <div className="mb-4">
        ...
        // Collapsible sections use click handler
        return (
            <div className="mb-4">
    ...
    return (
        <div className="container-fluid min-vh-100">
            ...
            {/* --- BOQ Summary Container (FIXED: Removed manual positioning, set radius for seamless look) --- */}
            <div 
                className="text-white p-3 d-flex justify-content-between align-items-center mx-3" 
                style={{ 
                    // This uses the Bootstrap mx-3 (margin-left/right) to align with the form below
                    maxWidth: '98%', // Keeping max-width for consistency
                    // REMOVED: marginLeft: 'auto', maxWidth: '93%'
                    background: `linear-gradient(to right, ${darkBlue}, ${vibrantBlue})`, 
                    borderRadius: '0.5rem 0.5rem 0 0', // Top corners rounded, bottom corners square
                }} 
            >
                <div className="d-flex align-items-center">
            ...
            {/* --- Main Content Area (Uses the same mx-3 width/margin as the BOQ Summary) --- */}
            <div 
                ...
                style={{ 
                    ...
                    borderRadius: '0 0 0.5rem 0.5rem',
                    border: `1px solid ${containerBorderColor}`, 
                    borderTop: 'none', // Critical: Connects seamlessly to the BOQ Summary
