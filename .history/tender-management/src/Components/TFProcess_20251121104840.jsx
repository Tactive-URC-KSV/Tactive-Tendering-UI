...
import { Paperclip, ArrowUp, ArrowRight, X } from 'lucide-react'; // Added 'X' for file removal

// --- 1. UploadBox Component Definition ---
// This component handles the UI for a single attachment and manages the file/notes state.
const UploadBox = ({ title, placeholder, notesPlaceholder, file, setFile, notes, setNotes }) => {
    ...
    const handleFileChange = (event) => {
        ...
        // Reset file input value so the same file can be selected again after deletion
    ...
    return (
        <div className="col-md-6 mb-4">
            <div className="border rounded p-3" style={{ border: '2px dashed #ced4da', minHeight: '220px' }}>
                <div className="text-center">
                    <p className="fw-bold mb-3 mt-2">{title}</p>
                    
                    {/* File Display Area */}
                    {file ? (
                        // Display filename if file is present
                        <div className='py-4 px-3 d-flex align-items-center justify-content-between'
                             style={{ border: '1px solid #2BA95A', borderRadius: '4px' }}
                        >
                        ...
                        // Upload area if no file is present
                        <div 
                            className="py-4 px-3" 
                            style={{ border: '1px dashed #005197', borderRadius: '4px', cursor: 'pointer' }}
                            onClick={handleClick} 
                        >
                            <ArrowUp size={24} color="#005197" style={{ marginBottom: '5px' }} />
                            <span className="d-block text-muted small">{placeholder}</span>
                        </div>
                    )}
...
// --- 2. attachmentDetails Component Definition ---
// This is the main component that renders the attachment section.
const attachmentDetails = ({ handlePrevious, setCurrentTab }) => { 
    // State Management for Attachments and Notes
    ...
    const [drawingsNotes, setDrawingsNotes] = useState('');
    ...
    // Others
    const [othersFile, setOthersFile] = useState(null);
    ...
    return (
        <div className="p-2">
            ...
            <div className="container mt-4">
                <div className="row">
                    <UploadBox 
                        ...
                        notesPlaceholder="Additional notes for technical specification"
                        file={techSpecFile}
                        setFile={setTechSpecFile}
                        notes={techSpecNotes}
                        setNotes={setTechSpecNotes}
                    />
                    <UploadBox 
                        title="Drawings" 
                        placeholder="Click to upload or drag and drop"
                        notesPlaceholder="Additional notes for Drawings"
                        file={drawingsFile}
                        setFile={setDrawingsFile}
                        notes={drawingsNotes}
                        setNotes={setDrawingsNotes}
                    />
                </div>
                <div className="row">
                    <UploadBox 
                        ...
                        placeholder="Click to upload or drag and drop"
                        notesPlaceholder="Additional notes for Commercial Conditions"
                        file={commercialFile}
                        setFile={setCommercialFile}
                        notes={commercialNotes}
                        setNotes={setCommercialNotes}
                    />
                    <UploadBox 
                        title="Others" 
                        placeholder="Click to upload or drag and drop"
                        notesPlaceholder="Additional notes for others"
                        file={othersFile}
                        setFile={setOthersFile}
                        notes={othersNotes}
            ...
            <div className="d-flex justify-content-between align-items-center ms-3 me-3 mt-4 mb-4">
                {/* Note: handlePrevious and setCurrentTab must be passed as props from the parent component */}
                <button className="btn cancel-button" onClick={handlePrevious}>Previous</button>
                <button className="btn action-button" onClick={() => setCurrentTab('review')}>
...
// You should export the component you want to use in your main file
// export { attachmentDetails }; 
// If attachmentDetails is defined inside another component, you would call it like:
// {attachmentDetails({ handlePrevious, setCurrentTab })}
