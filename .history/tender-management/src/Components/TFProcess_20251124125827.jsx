...
<div className="p-4 mt-5 bg-white rounded-3" style={{ border: '1px solid #e0e0e0', boxShadow: '0 2px 4px rgba(0,0,0,.05)' }}>
    ...
    {/* 1. TENDER SUMMARY BOX (Light Blue Background, Two-Row Grid) */}
    <div 
        className="p-3 d-flex flex-column rounded-3" 
        style={{ 
            backgroundColor: '#F0F8FF', // Light Blue Background (from your current op)
            border: '1px solid #B0C4DE', // Light Blue Border (from your current op)
        }}
    >
        
        {/* Box Header: Tender Summary (Blue text, icon) */}
        <div className="d-flex align-items-center mb-3 fw-bold" style={{ color: '#005197' }}>
            <img src="/path/to/icon.png" alt="Tender Summary Icon" style={{ width: '20px', height: '20px', marginRight: '8px' }} /> 
            {/* Replace <img> with an actual icon component if using react-icons/feather, e.g., <Box size={20} /> */}
            Tender Summary
        </div>
        ...
        <div className="d-flex flex-column" style={{ color: '#005197', fontSize: '14px' }}> 
            ...
            <div className="d-flex gap-5 mb-2"> 
                ...
                <div style={{ width: '200px' }}> 
                    Total Packages: <span className="fw-bold" style={{ color: '#333' }}>{boqArray.length > 0 ? 1 : 0}</span> 
                </div> 

                {/* Selected Contractors */}
                <div style={{ width: '200px' }}>
                    Selected Contractors: <span className="fw-bold" style={{ color: '#333' }}>{contractorsList.length}</span>
                </div>
            </div>
            ...
            <div className="d-flex gap-5">
                {/* Attachments */}
                <div style={{ width: '200px' }}>
                    Attachments: <span className="fw-bold" style={{ color: '#333' }}>{Object.keys(attachments).filter(key => attachments[key].files?.length > 0 || attachments[key].notes).length}</span>
                </div>
                ...
                <div style={{ width: '200px' }}>
                    ...
                    <span className="fw-bold" 
                        style={{ 
                            // Red if 'Not Set' or if date is set (matching the red '10 days' in the preview)
                            color: tenderDetail.offerSubmissionDate ? '#dc3545' : '#dc3545' 
                        }}
                    >
