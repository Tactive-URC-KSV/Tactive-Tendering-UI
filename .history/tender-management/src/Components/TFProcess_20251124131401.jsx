...
<div className="mt-5 mb-5">
    ...
    {/* 1. TENDER SUMMARY BOX (Single Row) */}
    <div 
    ...
    >
        ...
        {/* Statistics Section - Single Row, using gap for spacing */}
        <div className="d-flex gap-5" style={{ color: '#005197', fontSize: '14px' }}> 
            
            {/* Total Packages */}
            <div className="d-flex gap-1"> 
                <span>Total Packages:</span> <span>{boqArray.length > 0 ? 1 : 0}</span> 
            </div> 

            {/* Selected Contractors */}
            <div className="d-flex gap-1">
                <span>Selected Contractors:</span> <span>{contractorsList.length}</span> 
            </div>

            {/* Attachments */}
            <div className="d-flex gap-1">
                <span>Attachments:</span> <span>{Object.keys(attachments).filter(key => attachments[key].files?.length > 0 || attachments[key].notes).length}</span>
            </div>
