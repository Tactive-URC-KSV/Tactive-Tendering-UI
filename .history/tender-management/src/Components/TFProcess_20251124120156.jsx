...
<div className="p-4 mt-5 bg-white rounded-3" style={{ border: '1px solid #e0e0e0', boxShadow: '0 2px 4px rgba(0,0,0,.05)' }}>
    ...
    <div className="ms-1 me-1 p-3" style={{ border: '1px solid #eee', borderRadius: '8px' }}>
        {Object.keys(attachments).map((key) => {
            ...
            // Assuming the text like 'jh', 'sf', 'asfigf' in your current output is stored in 'notes'
            const notes = attachmentData.notes || ''; 
            
            // Assuming files[0] holds the file information (type, size)
            const fileInfo = files.length > 0 ? files[0] : null;
            
            return (
                // Use py-3 for vertical spacing and border-bottom for row separation
                <div key={key} className="d-flex justify-content-between align-items-center py-3 border-bottom" style={{ borderColor: '#eee' }}>
                    <div className="text-start">
                        {/* 1. Title (e.g., Technical Specifications) */}
                        <span className="fw-medium d-block mb-1" style={{ color: '#333' }}>{title}</span>
                        
                        {/* 2. File Type/Size/Details (RED COLOR, like 'image/jpeg | 157797') */}
                        {fileInfo && (
                            <span className="fw-medium me-2" style={{ fontSize: '13px', color: '#dc3545' }}>
                                {/* Display file type, size/ID/placeholder for file */}
                                {fileInfo.type || fileInfo.extension || 'File'} | {fileInfo.size || fileInfo.id || 'N/A'}
                            </span>
                        )}
                        
                        {/* 3. Description/Notes (Muted color, like 'jh', 'sf', etc.) */}
                        <span className="text-muted d-block mt-1" style={{ fontSize: '13px' }}>
                            {notes}
                        </span>
                    </div>
                    
                    {/* 4. Download Icon (Right side) - Only show if files are present */}
                    {fileInfo && (
                        <span 
                            className="p-2 rounded-circle" 
                            style={{ cursor: 'pointer', color: '#005197', backgroundColor: '#F0F8FF' }}
                            // onClick={handleDownload(fileInfo)} // Add your actual download handler here
                        >
