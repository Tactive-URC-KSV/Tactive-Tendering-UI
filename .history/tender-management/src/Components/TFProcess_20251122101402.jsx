...
const handleNoteChange = (key, value) => {
    ...
    const reviewBoqs = mockBoqReview;
    const reviewAttachments = {
        technical: { fileName: "Technical Specifications.pdf", size: "4.7 MB" },
        drawings: { fileName: "Drawings.png", size: "1.2 MB" },
        commercial: { fileName: "Commercial Conditions.xlsx", size: "6.2 MB" },
        others: { fileName: "Others.pdf", size: "4.2 MB" }
    }

    const AttachmentRow = ({ title, fileName, size, download }) => (
        <div className="d-flex justify-content-between py-2" style={{ borderBottom: '1px solid #eee' }}>
            <div className="text-start">
                ...
                <p className="text-muted mb-0" style={{ fontSize: '13px' }}>{download}</p>
            </div>
            <div className="d-flex align-items-center">
                <span className="me-2 text-muted" style={{ fontSize: '14px' }}>
                    {fileName} ({size})
                </span>
                <span className="download-icon" style={{ cursor: 'pointer', color: '#005197' }}>
                    <Download size={18} /> 
                </span>
            </div>
        </div>
        ...
        <div className="p-4">
            ...
            <div className="d-flex justify-content-start gap-5 ms-1" style={{ borderBottom: '1px solid #00519933', paddingBottom: '15px' }}>
                <div>
                    ...
                    <span className="fw-bold" style={{ color: '#005197', fontSize: '16px' }}>{reviewData.tenderFloatingNo}</span>
                </div>
                <div>
                    <span className="text-muted d-block" style={{ fontSize: '14px' }}>Floating Date</span>
                    <span className="fw-bold" style={{ color: '#333', fontSize: '16px' }}>{reviewData.floatingDate}</span>
                </div>
            </div>
            ...
            <div className="pt-4">
                ...
                <div className="row g-4 ms-1">
                    <div className="col-md-4 text-start">
                        ...
                        <span className="fw-medium">{project?.projectName || reviewData.projectName}</span>
                    </div>
                    <div className="col-md-4 text-start">
                        <span className="text-muted d-block">Offer Submission Mode</span>
                        <span className="fw-medium">{reviewData.offerSubmissionMode}</span>
                    </div>
                    <div className="col-md-4 text-start">
                        <span className="text-muted d-block">Submission Last Date</span>
                        <span className="fw-medium" style={{color: '#dc3545'}}>{reviewData.submissionLastDate}</span>
                    </div>
                    <div className="col-md-4 text-start">
                        <span className="text-muted d-block">Bid Opening</span>
                        <span className="fw-medium">{reviewData.bidOpening}</span>
                    </div>
                    <div className="col-md-4 text-start">
                        <span className="text-muted d-block">Contact Person</span>
                        <span className="fw-medium">{reviewData.contactPerson}</span>
                    </div>
                    <div className="col-md-4 text-start">
                        <span className="text-muted d-block">Contact Number</span>
                        <span className="fw-medium">{reviewData.contactNumber}</span>
                    </div>
                    <div className="col-md-4 text-start">
                        <span className="text-muted d-block">Contact Email ID</span>
                        <span className="fw-medium">{reviewData.contactEmail}</span>
                    </div>
                    <div className="col-md-8 text-start">
                        <span className="text-muted d-block">Scope of Work</span>
                        <span className="fw-medium">{reviewData.scopeOfWork}</span>
                    </div>
                    <div className="col-md-12 text-start">
                        <span className="text-muted d-block">Scope of Packages</span>
                        <div className="d-flex flex-wrap gap-2 mt-1">
                            {reviewData.scopeOfPackages.map((scope, index) => (
                ...
                <div className="table-responsive ms-1 me-1">
                    <table className="table table-borderless table-striped">
                        ...
                        <tbody>
                            {reviewBoqs.map((boq, index) => (
                                <tr key={index}>
                                    <td>{boq.boqCode}</td>
                                    <td title={boq.boqName}>{boqNameDisplay(boq.boqName)}</td>
                                    <td>{boq.unit}</td>
                                    <td className="text-end">{boq.quantity.toFixed(4)}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                ...
                <div className="text-start ms-1 mt-5 mb-3">
                    ...
                    <p className="text-muted mt-1" style={{ fontSize: '14px' }}>{reviewData.contractors.length} contractors selected for tender invitation</p>
                </div>
                <div className="d-flex flex-wrap gap-3 ms-1">
                    {reviewData.contractors.map((contractor, index) => (
                        <div key={index} className="p-3" style={{ border: '1px solid #ccc', borderRadius: '8px', minWidth: '300px' }}>
                            <div className="d-flex justify-content-between align-items-start">
                                ...
                                {/* Remove icon from the screenshot */}
                ...
                <div className="ms-1 me-1 p-3" style={{ border: '1px solid #eee', borderRadius: '8px' }}>
                    <AttachmentRow 
                        ...
                        fileName={reviewAttachments.technical.fileName} 
                        size={reviewAttachments.technical.size} 
                        download="Detailed technical requirements for all construction phases including material specifications and quality standards." 
                    />
                    <AttachmentRow 
                        title="Drawings" 
                        fileName={reviewAttachments.drawings.fileName} 
                        size={reviewAttachments.drawings.size} 
                        download="Complete set of architectural and engineering drawings including structural, electrical, and HVAC layouts." 
                    />
                    <AttachmentRow 
                        title="Commercial Conditions" 
                        fileName={reviewAttachments.commercial.fileName} 
                        size={reviewAttachments.commercial.size} 
                        download="Payment terms, penalties, insurance requirements, and other commercial conditions for the tender." 
                    />
                    <AttachmentRow 
                        title="Others" 
                        fileName={reviewAttachments.others.fileName} 
                        size={reviewAttachments.others.size} 
                        download="Safety protocols and environmental compliance requirements." 
                    />
                </div>
                ...
                <div className="p-3 mt-5 mb-5 d-flex justify-content-between align-items-center" style={{ backgroundColor: '#F0F8FF', border: '1px solid #B0C4DE', borderRadius: '8px' }}>
                    <div className="d-flex gap-5 fw-bold" style={{ color: '#005197' }}>
                        <div>Total Packages: <span style={{ color: '#333' }}>{reviewData.totalPackages}</span></div>
                        <div>Selected Contractors: <span style={{ color: '#333' }}>{reviewData.selectedContractors}</span></div>
                        <div>Attachments: <span style={{ color: '#333' }}>{Object.keys(reviewAttachments).length}</span></div>
                        <div>Submission Deadline: <span style={{ color: '#dc3545' }}>{reviewData.submissionDeadlineDays} days</span></div>
                    </div>
                    <button className="btn fw-bold" style={{ backgroundColor: '#2BA95A', color: 'white', padding: '10px 20px' }}>
                        Float Tender
                    </button>
                </div>
            </div>
