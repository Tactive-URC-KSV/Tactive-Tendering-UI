...
function TFProcess({ projectId }) {
    ...
    bidOpeningDate: '01-12-2025', 
    offerSubmissionDate: '30-11-2025', 
    contactPerson: 'Sarah Mitchell', 
    contactEmail: 'sarah.mitchell@greenheights.com', 
    contactMobile: '+1 (555) 123-4567', 
    ...
    floatingNo: 'TF - 2025 - 001',
    floatingDate: 'November 20, 2025',
    });
...
const handleNoteChange = (key, value) => {
    ...
    // --- File: TFProcess.js (Near the top of the function) ---

const [selectedContractors, setSelectedContractors] = useState([
    { id: 1, name: "Build Tech Solutions", contactPerson: "John Smith", email: "john@buildtech.com" },
    { id: 2, name: "Electro Tech Inc", contactPerson: "Sarah Johnson", email: "sarah.johnson@electrotech.com" }
]);
...
const project = {
   ...
   // --- File: TFProcess.js (Place this function before renderContent) ---

const reviewTender = () => {
    // Get the BOQ array (already defined in your code's scope)
    const selectedBoqArray = getSelectedLeafBoqs(parentTree); 

    // Helper to format BOQ name (already defined in your code's scope)
    const boqNameDisplay = (boqName) => {
        return boqName && boqName.length > 20
            ? boqName.substring(0, 20) + '...'
            : boqName;
    ...
    // Determine the number of unique attachment types with files
   const attachmentCount = Object.values(attachments).filter(
    (att) => att.files.length > 0
).length; // <-- Ensure this line is correctly terminated.
    
    // Static or calculated submission deadline
    const submissionDeadline = "10 days"; 

    return (
        <div className="p-4">
            {/* Review & Float Tender Header */}
            <div className="text-start ms-1 mt-3">
                <Check size={20} color="#005197" />
                <span className="ms-2 fw-bold" style={{ color: "#005197" }}>Review & Float Tender</span>
                <p className="text-muted ms-1 mt-2" style={{ fontSize: "14px" }}>
                    Review tender details and float to selected contractors
                </p>
            </div>

            {/* Tender Floating No & Floating Date */}
            <div className="row align-items-center ms-1 mb-5 p-3" style={{ border: '1px solid #0051973D', borderRadius: '8px' }}>
                <div className="col-md-6 text-start">
                    <label className="projectform d-block fw-bold">Tender Floating No</label>
                    <span className="text-muted">{tenderDetail.floatingNo || 'TF - 2025 - 001'}</span>
                </div>
                <div className="col-md-6 text-start">
                    <label className="projectform d-block fw-bold">Floating Date</label>
                    <span className="text-muted">{tenderDetail.floatingDate || 'November 20, 2025'}</span>
                </div>
            </div>

            {/* General Details Summary */}
            <div className="text-start ms-1 mt-4">
                <Info size={20} color="#005197" />
                <span className="ms-2 fw-bold" style={{ color: "#005197" }}>General Details</span>
            </div>
            <div className="row align-items-center ms-1 mt-4">
                <div className="col-md-4 text-start">
                    <label className="projectform text-start d-block fw-bold">Project Name</label>
                    <span className="text-muted">{project?.projectName || 'N/A'}</span>
                </div>
                <div className="col-md-4 text-start">
                    <label className="projectform text-start d-block fw-bold">Offer Submission Mode</label>
                    <span className="text-muted">{tenderDetail.offerSubmissionMode ? tenderDetail.offerSubmissionMode.charAt(0).toUpperCase() + tenderDetail.offerSubmissionMode.slice(1) : 'N/A'}</span>
                </div>
                <div className="col-md-4 text-start">
                    <label className="projectform text-start d-block fw-bold">Submission Last Date</label>
                    <span className="text-danger fw-bold">{tenderDetail.offerSubmissionDate || 'N/A'}</span>
                </div>
            </div>
            <div className="row align-items-center ms-1 mt-4">
                <div className="col-md-4 text-start">
                    <label className="projectform text-start d-block fw-bold">Bid Opening</label>
                    <span className="text-muted">{tenderDetail.bidOpeningDate || 'N/A'}</span>
                </div>
                <div className="col-md-4 text-start">
                    <label className="projectform text-start d-block fw-bold">Contact Person</label>
                    <span className="text-muted">{tenderDetail.contactPerson || 'N/A'}</span>
                </div>
                <div className="col-md-4 text-start">
                    <label className="projectform text-start d-block fw-bold">Contact Number</label>
                    <span className="text-muted">{tenderDetail.contactMobile || 'N/A'}</span>
                </div>
            </div>
            <div className="row align-items-center ms-1 mt-4">
                <div className="col-md-4 text-start">
                    <label className="projectform text-start d-block fw-bold">Contact Email ID</label>
                    <span className="text-muted">{tenderDetail.contactEmail || 'N/A'}</span>
                </div>
            </div>
            <div className="row align-items-center ms-1 mt-4">
                <div className="col-md-12 text-start">
                    <label className="projectform text-start d-block fw-bold">Scope of Work</label>
                    <span className="text-muted">{tenderDetail.scopeOfWork || 'Complete construction of 5-story commercial complex including foundation, structure, MEP systems, and finishing works'}</span>
                </div>
            </div>
            <div className="row align-items-center ms-1 mt-4">
                <div className="col-md-12 text-start">
                    <label className="projectform text-start d-block fw-bold">Scope of Packages</label>
                    <div className="mt-2">
                        {selectedScopes.length > 0 ? ( 
                            selectedScopes.map((scopeValue, index) => {
                                // Find the matching label from scopeOptions to display it nicely
                                const scopeOption = scopeOptions.find(opt => opt.value === scopeValue);
                                return (
                                    <span key={index} className="badge me-2 py-2 px-3" style={{ backgroundColor: '#B0C4DE', color: '#005197', fontWeight: 'normal' }}>
                                        {scopeOption ? scopeOption.label : scopeValue}
                                    </span>
                                );
                            })
                        ) : (
            ...
            {/* --- Separator --- */}
            <hr className="my-5" style={{ borderColor: '#0051973D' }} />

            {/* Package Details Summary (BOQ Table) */}
            <div className="text-start ms-1 mt-4">
                <BoxesIcon size={20} color="#005197" />
                <span className="ms-2 fw-bold" style={{ color: "#005197" }}>Package Details</span>
            </div>
            <div className="table table-responsive mt-4">
                <table className="table table-borderless">
                    <thead style={{ color: '#005197' }}>
                        <tr>
                            <th style={{ width: '15%' }}>BOQ Code</th>
                            <th style={{ width: '45%' }}>BOQ Name</th>
                            <th style={{ width: '20%' }}>Unit</th>
                            <th style={{ width: '20%' }}>Quantity</th>
                        </tr>
                    </thead>
                    <tbody>
                        {selectedBoqArray && selectedBoqArray.length > 0 ? (
                            selectedBoqArray.map((boq) => (
                                <tr key={boq.id} style={{ borderBottom: '1px solid #eee' }}>
                                    <td className='text-muted' style={{ fontSize: '14px' }}>{boq.boqCode}</td>
                                    <td className='text-muted' style={{ fontSize: '14px' }} title={boq.boqName}>{boqNameDisplay(boq.boqName) || '-'}</td>
                                    <td className='text-muted' style={{ fontSize: '14px' }}>{boq.uom?.uomCode || 'N/A'}</td>
                                    <td className='text-muted' style={{ fontSize: '14px' }}>{boq.quantity?.toFixed(3) || 0}</td>
                                </tr>
                            ))
                        ) : (
                            <tr>
                                <td colSpan="4" className="text-center text-muted py-4">No BOQ items selected.</td>
                            </tr>
                        )}
                    </tbody>
                </table>
            ...
            <div className="text-start ms-1 mt-4">
                <User2 size={20} color="#005197" />
                <span className="ms-2 fw-bold" style={{ color: "#005197" }}>Contractor Details</span>
                <p className="text-muted ms-1 mt-2" style={{ fontSize: "14px" }}>
                    {selectedContractors?.length || 0} contractors selected for tender invitation
                </p>
            </div>
            <div className="d-flex flex-wrap ms-1 mt-3 gap-3">
                {selectedContractors && selectedContractors.length > 0 ? (
                    selectedContractors.map((contractor) => (
                        <div key={contractor.id} className="p-3" style={{ border: '1px solid #B0C4DE', borderRadius: '8px', minWidth: '300px', textAlign: 'left' }}>
                            <div className="fw-bold">{contractor.name}</div>
                            <div className="text-muted" style={{ fontSize: '14px' }}>{contractor.contactPerson} &bull; {contractor.email}</div>
                        </div>
                    ))
                ) : (
                    <span className="text-muted ms-1">No contractors selected.</span>
                )}
            </div>
            ...
            <div className="row ms-1 mt-4">
                {Object.keys(attachments).map((key) => {
                    ...
                    const typeTitle = titleMap[key] || key.charAt(0).toUpperCase() + key.slice(1);
                    const fileCount = attachments[key].files.length;
                    const notes = attachments[key].notes;
                    
                    if (fileCount === 0 && !notes) return null;

                    return (
                        <div key={key} className="col-md-6 mb-3 text-start">
                             <div className="fw-bold" style={{ color: "#333" }}>{typeTitle}</div>
                            <ul className="list-unstyled mt-1" style={{ fontSize: '14px' }}>
                                {attachments[key].files.map((file, index) => (
                                    // You might need a helper to get file size/type
                                    <li key={index} className="text-muted">📄 {file.name}</li>
                                ))}
                                {notes && <li className="text-muted">📝 *Notes: {notes}</li>}
                                {fileCount === 0 && notes && <li className="text-muted">*Only notes provided.</li>}
                            </ul>
                        </div>
                    );
                })}
            ...
            {/* Tender Summary Footer */}
            <div className="text-start ms-1 mt-4">
                <span className="fw-bold" style={{ color: "#005197" }}>Tender Summary</span>
            </div>
            <div className="d-flex align-items-center justify-content-start ms-1 mt-3 gap-5 p-3" style={{ border: '1px solid #0051973D', borderRadius: '8px' }}>
                <div className="text-start">
                    <div className="text-muted" style={{ fontSize: '14px' }}>Total Packages</div>
                    <div className="fw-bold">{selectedBoqArray?.length || 0}</div>
                </div>
                <div className="text-start">
                    <div className="text-muted" style={{ fontSize: '14px' }}>Attachments</div>
                    <div className="fw-bold">{attachmentCount}</div>
                </div>
                <div className="text-start">
                    <div className="text-muted" style={{ fontSize: '14px' }}>Selected Contractors</div>
                    <div className="fw-bold">{selectedContractors?.length || 0}</div>
                </div>
                <div className="text-start">
                    <div className="text-muted" style={{ fontSize: '14px' }}>Submission Deadline</div>
                    <div className="fw-bold text-danger">{submissionDeadline}</div>
                </div>
            </div>
            ...
            <div className="d-flex justify-content-end align-items-center mt-5">
                <button 
                    className="btn me-3" 
                    onClick={() => setCurrentTab('tender')}
                    style={{ border: '1px solid #005197', color: '#005197', backgroundColor: 'white', fontWeight: 'bold' }}
                >
                    Start editing
                </button>
                <button 
