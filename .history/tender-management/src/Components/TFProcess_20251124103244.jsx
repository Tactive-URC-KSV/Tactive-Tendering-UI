...
const AttachmentRow = ({ title, files, notes }) => (
  ...
  const reviewTender = () => {
       ...
       // Assume necessary icons (Info, BoxesIcon, User2, Paperclip, ArrowLeft, ArrowRight, X) 
// and helper functions (getAttachmentTitle, boqNameDisplay) are imported/defined elsewhere.

// --- UTILITY FUNCTIONS REQUIRED FOR reviewTender ---
// NOTE: These must be defined outside or inside the main component scope before reviewTender.

// Utility to format the date as M/D/YYYY (e.g., 12/3/2025)
const formatScreenshotDate = (dateString) => {
    if (!dateString) return 'N/A';
    try {
        // Using toLocaleDateString() without arguments often defaults to M/D/YYYY or D/M/YYYY
        // For strict M/D/YYYY as per screenshot, using 'en-US' locale is safer.
...
// Utility to format the floating date (e.g., November 24, 2025)
...
// Assuming getDisplayMode is defined elsewhere
// const getDisplayMode = (mode) => ...

// --- REVIEW TENDER FUNCTION (FULLY UPDATED) ---

const reviewTender = () => {
    // Placeholder declarations for data used in reviewTender (Assume they are props/state/context)
    const boqArray = []; // Replace with actual boq list
    const contractorsList = []; // Replace with actual contractors list
    const attachments = {}; // Replace with actual attachments object
    const project = {}; // Replace with actual project object
    const tenderDetail = { // Replace with actual tender details object
        tenderFloatingNo: 'TF - 948 - 001',
        floatingDate: '2025-11-24',
        submissionLastDate: '2025-12-03',
        bidOpeningDate: '2025-12-05',
        // ... other fields ...
    };
    const scopeOptions = []; // Replace with actual scope options

    // Utility to count active attachments for the Tender Summary
    ...
    return (
        <div className="p-4">
            ...
            {/* Tender Floating No and Floating Date - Aligned Left/Right (as requested earlier) */}
            <div className="d-flex justify-content-between mb-4">
                ...
                <div className="text-end">
                    ...
                    <span className="fw-medium" style={{ fontSize: '16px' }}>
                        {formatFloatingDate(tenderDetail.floatingDate)}
                    </span>
                </div>
            </div>
            
            {/* --- Start of Content Sections with White Bordered Boxes --- */}
            
            <div className="pt-4">
                
                {/* 1. General Details Section (With White Bordered Box) */}
                <div className="p-4 mb-4" style={{ border: '1px solid #e0e0e0', borderRadius: '8px', backgroundColor: 'white' }}>
                    ...
                    <div className="row g-4 ms-1">
                        ...
                        <div className="col-md-4 text-start">
                            ...
                            <span 
                                ...
                                style={{ color: '#dc3545' }} 
                            >
                                {formatScreenshotDate(tenderDetail.submissionLastDate)} 
                            </span>
                        </div>
                        
                        <div className="col-md-4 text-start">
                            <span className="text-muted d-block">Bid Opening</span>
                            <span className="fw-medium">{formatScreenshotDate(tenderDetail.bidOpeningDate)} </span>
                        </div>
                        ...
                        {/* Scope of Work (col-md-12 for new line) */}
                ...
                {/* 2. Package Details Section (With White Bordered Box) */}
                <div className="p-4 mb-4" style={{ border: '1px solid #e0e0e0', borderRadius: '8px', backgroundColor: 'white' }}>
                ...
                {/* 3. Contractor Details Section (With White Bordered Box) */}
                <div className="p-4 mb-4" style={{ border: '1px solid #e0e0e0', borderRadius: '8px', backgroundColor: 'white' }}>
                    <div className="text-start ms-1 mt-5 mb-3">
                ...
                {/* 4. Attachments Section (With White Bordered Box) */}
                <div className="p-4 mb-4" style={{ border: '1px solid #e0e0e0', borderRadius: '8px', backgroundColor: 'white' }}>
                    <div className="text-start ms-1 mt-5 mb-3">
                        <Paperclip size={20} color="#005197" />
                        <span className="ms-2 fw-bold" style={{ color: '#005197' }}>Attachments</span>
                ...
                {/* Tender Summary Footer (COMPACT LAYOUT) */}
                <div className="p-3 mt-5 mb-5 d-flex justify-content-between align-items-center" style={{ backgroundColor: '#F0F8FF', border: '1px solid #B0C4DE', borderRadius: '8px' }}>
                    <div className="d-flex gap-4 fw-bold" style={{ color: '#005197', fontSize: '14px' }}>
                        
                        <span>Total Packages: <span className="text-dark">{boqArray.length > 0 ? 1 : 0}</span></span> 
                        
                        <span>Selected Contractors: <span className="text-dark">{contractorsList.length}</span></span>
                        
                        <span>Attachments: <span className="text-dark">{activeAttachmentCount}</span></span>
                        
                        <span style={{ marginLeft: '10px' }}>
                            Submission Deadline: 
                            <span style={{ color: '#dc3545' }}>
                                {tenderDetail.submissionLastDate ? formatScreenshotDate(tenderDetail.submissionLastDate) : 'Not Set'}
                            </span>
                        </span> 
                    </div>
                    <button 
...
// --- TFProcess Main Component (No changes to structure) ---

const TFProcess = () => {
    // Assume useState, boqSelection, tenderDetails, etc. are defined/imported
    const [currentTab, setCurrentTab] = useState('review'); // Set initial tab for testing review
    // Placeholder data (must be defined in your actual component)
    const project = { projectName: 'My Project', projectCode: 'P001' };
    const selectedBoq = { size: 5 }; 
    
    // Placeholder functions (must be defined in your actual component)
    const boqSelection = () => <div>BOQ Selection Content</div>;
    const tenderDetails = () => <div>Tender Details Content</div>;
