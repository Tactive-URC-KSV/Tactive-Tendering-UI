...
// Assuming you are using 'lucide-react' for icons and 'react-select'
import { Paperclip, Boxes as BoxesIcon, User2, Info, Plus, X, ArrowRight, ArrowLeft } from 'lucide-react';
import Select from 'react-select'; 
// Note: You must ensure 'react-select' and 'lucide-react' are installed:
// npm install react-select lucide-react

const TFProcess = () => {
    // ------------------------------------------
    // 1. STATE MANAGEMENT (MOCK/EXAMPLE DATA)
    // ------------------------------------------
    
    // Main workflow state: 'boq', 'tender', or 'review'
    const [currentTab, setCurrentTab] = useState('boq'); 
    
    // Tender Details sub-tab state: 'general', 'package', 'contractor', or 'attachment'
    const [tab, setTab] = useState('general'); 

    // BOQ Selection state
    const [selectedBoq, setSelectedBoq] = useState(new Set([1, 2])); 
    const [boqForRemoval, setBoqForRemoval] = useState(new Set());
    const [parentTree, setParentTree] = useState([]); // Placeholder for BOQ hierarchy
    ...
    // Attachment state (File objects and display info)
    ...
    // Project Info
    const project = { projectName: 'Building Project', projectCode: 'BP-2025' };
    
    // Scope Options for the Select component
    const scopeOptions = [
    ...
    // ------------------------------------------
    // 2. HELPER FUNCTIONS (MOCK)
    // ------------------------------------------

    // Mock function to extract selected leaf BOQs
    const getSelectedLeafBoqs = () => {
        return [
            { id: 1, boqCode: 'B001', boqName: 'Concrete Slab Work, a very long name for an item', uom: { uomCode: 'M3' }, quantity: 150.555 },
            { id: 2, boqCode: 'B002', boqName: 'Steel Rebar Supply and Installation', uom: { uomCode: 'KG' }, quantity: 5000.000 },
        ].filter(boq => selectedBoq.has(boq.id));
    };
    
    // Handles toggling a single BOQ for removal selection
    ...
    // Handles removal of selected BOQs from the main list
    ...
    // Handles file upload and updates attachmentFiles state
    ...
    // Handles file removal from attachmentFiles state
    ...
    const contractorDetails = () => {
        return (
            <div className="p-2">
                ...
                {/* Contractor selection implementation goes here */}
            </div>
        );
    }
    ...
    const tenderDetails = () => {
        ...
        // Internal navigation for Step 2
        const handleNext = () => {
            if (tab === 'general') {
                setTab('package');
            }
        ...
        return (
            <div className="bg-white ms-3 me-3 rounded-3 p-3 mt-5" style={{ border: '1px solid #0051973D' }}>
                ...
                {/* Internal Next/Previous buttons for sub-tabs */}
    ...
    // Placeholder for the BOQ selection tree component
    const boqSelection = () => {
        return (
            <div className="p-2">
                <div className="text-center text-muted py-5">BOQ Selection Tree/Table component goes here.</div>
                {/* This content is replaced by the tree/table in a real app */}
            </div>
        );
    }
    ...
    // Main navigation for the 3-step process
