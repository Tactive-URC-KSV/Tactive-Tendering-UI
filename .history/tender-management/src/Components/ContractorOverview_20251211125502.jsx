...
const [formData, setFormData] = useState(loadFormData());
    ...
    // --- Validation configuration: only these fields are required for "manual" view
const requiredFields = {
  ...
  additionalInfoType: "Additional Info Type",
  registrationNo: "Registration No"
};

// helper to consider a value "filled"
const isFilled = (val) => {
...
// Compute missing fields and boolean validity (only when manual view)
const { missingFields, isFormValid } = useMemo(() => {
  const missing = [];
  if (selectedView === "manual") {
    for (const key of Object.keys(requiredFields)) {
      if (!isFilled(formData[key])) missing.push(requiredFields[key]);
    }
  } else {
    // For email view we don't require manual fields; you can add checks for contractorEmailId if desired
  }
  return { missingFields: missing, isFormValid: missing.length === 0 };
  ...
  e.preventDefault();
  if (!isFormValid) {
    console.warn("Cannot submit — missing:", missingFields);
    // optionally show a toast or focus first missing field
    return;
  }
    ...
    return (
        <div className="container-fluid min-vh-100 p-0">
            ...
            <div className="px-4 text-start">
                ...
                {selectedView === "manual" && !isFormValid && (
  <div className="mb-3 p-3 rounded" style={{ background: "#FFF7E6", color: "#7A4A00" }}>
    <strong>Please fill these required fields:</strong>
    <ul className="mb-0">
      {missingFields.map((f, i) => <li key={i}>{f}</li>)}
                ...
                {selectedView === 'manual' && (
                    <div className="d-flex justify-content-end mt-4">
                        ...
                        <button type="submit" form="contractorForm" className="btn px-4 fw-bold" style={{ backgroundColor: isFormValid ? bluePrimary : "#E6EEF9",color: isFormValid ? "white" : "#8FB4DD", borderRadius: "6px", borderColor: bluePrimary, cursor: isFormValid ? "pointer" : "not-allowed" }} disabled={!isFormValid}>Review & Submit<ArrowRight size={21} color={isFormValid ? "white" : "#8FB4DD"} className="ms-2"  /></button>
                    </div>
                )}
            </div>
