...
function AddResource() {
    ...
    // 1. SCROLL FIX: Ref to the main container
    const scrollRef = useRef(null);
    // 2. SCROLL FIX: State to track if an input change is causing a re-render
    const [isChanging, setIsChanging] = useState(false);
    // 3. SCROLL FIX: Ref to store scroll position
    const scrollPosition = useRef(0);

    // 4. SCROLL FIX: useLayoutEffect to capture and restore scroll position
    useLayoutEffect(() => {
    ...
    }, [isChanging]); // Depend on isChanging to trigger restoration
    ...
    const handleChange = (e) => {
        // SCROLL FIX: Store the current scroll position before state update
        ...
        // Note: setIsChanging(false) is done in useLayoutEffect after render
    };
    ...
    const toggleSection = (title) => {
        // Note: We don't use isChanging for section toggles as the scroll behavior is usually desired/less disruptive here.
    ...
    return (
        // 5. SCROLL FIX: Apply the ref to the main scrollable container
        <div className="container-fluid min-vh-100" ref={scrollRef}>
            ...
            {/* Keeping the form wrapper here is still good practice */}
            <form onSubmit={(e) => e.preventDefault()}> 
                ...
                {/* Wastage & Net Quantity (Dynamic/Collapsible) */}
                <FormSectionContainer 
                ...
                {/* Pricing & Currency (Dynamic/Collapsible) */}
                <FormSectionContainer 
