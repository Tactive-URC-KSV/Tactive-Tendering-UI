...
const ReviewSectionWrapper = ({ title, icon: Icon, children, subtitle }) => (
    ...
    <div className="card text-start border-0 shadow-sm mt-3" style={{ borderRadius: "4px", padding: "0 1.25rem 1.25rem 1.25rem" }}>
        {/* Blue Header Bar (Condensed padding: p-2) */}
        <div className="p-2 mb-3 d-flex align-items-center justify-content-start" style={{ backgroundColor: bluePrimary, width: "calc(100% + 2.5rem)", marginLeft: "-1.25rem", marginRight: "-1.25rem", borderRadius: "4px 4px 0 0" }}>
            <Icon size={16} className="ms-3 me-2 text-white" /> 
...
function ContractorReview() {
    // --- Mock Data Setup to Match the Screenshot Content ---
    const location = useLocation();
    // Using a comprehensive mock data object based on the latest screenshot
    ...
    const handleGoBack = () => navigate(-1); // For 'Start editing' button
    ...
    return (
        <div className="container-fluid min-vh-100 p-0" style={{ backgroundColor: '#f8f9fa' }}>
            {/* Main Content Card (Simulates the Figma/Web App boundary) */}
            <div className="bg-white mx-auto shadow" style={{ maxWidth: '900px' }}>
                
                {/* Fixed Top Bar (Tactive URC header area - implied by surrounding context) */}
                <div className="py-2 px-4 d-flex align-items-center" style={{ borderBottom: '1px solid #dee2e6' }}>
                ...
                <div className="px-4 pt-3 pb-5 text-start">
                    ...
                    <div className="card text-start border-0" style={{ borderRadius: "4px", padding: "1.25rem", borderLeft: '3px solid #28a745' }}>
