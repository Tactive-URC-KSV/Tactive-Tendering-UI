...
function AddResource() {
    ...
    const handleCalculations = useCallback((updatedData) => {
        setResourceData((prev) => {
            // Merge existing data with new data (including new rate/uomId if provided)
            ...
            const rate = parseFloat(data.rate) || 0; // Use the potentially new rate from updatedData
    ...
    // Triggers ONLY when selectedResource changes. This is the source of truth for the data.
    useEffect(() => {
        if (selectedResource) {
            const newRate = selectedResource.defaultRate || 0;
            const newUomId = selectedResource.defaultUomId || '';
            
            // This ASYNCHRONOUSLY updates the resourceData state, setting resourceData.rate and resourceData.uomId
            handleCalculations({ 
            ...
            console.log("--- EFFECT 1: Resource Selected and State Updated ---");
    ...
    useEffect(() => {
        ...
        if (uomOptions.length > 0 && currentUomId) {
            ...
            // Clear the visual UOM selection if the ID is empty
            setSelectedUom(null);
        }
                        ...
                        <div style={{ width: '80%' }}>
                            <Select
                                ...
                                onChange={(selected) => {
                                    ...
                                    // Directly update the uomId in the primary state
