...
function ContractorOverview() {
    ...
    // --- State Initialization Logic (UPDATED) ---
    ...
    // Function to load saved data from Session Storage
    const loadFormData = () => {
        try {
            ...
            if (savedData) {
                ...
                // NOTE: File objects in 'attachments' are excluded as they cannot be serialized.
                return { ...defaultFormData, ...parsedData, attachments: [] };
            }
        } catch (error) {
    ...
    // State initialization: Load from storage or use defaults
    const [selectedView, setSelectedView] = useState('manual');
    const [formData, setFormData] = useState(loadFormData());

    // --- Persistence Logic (NEW) ---
    // Save state to session storage whenever formData changes
    useEffect(() => {
        // Exclude the non-serializable 'attachments' array before saving
        const serializableData = { ...formData, attachments: [] }; 
        sessionStorage.setItem(STORAGE_KEY, JSON.stringify(serializableData));
    }, [formData]);
    
    // --- Handlers (Original logic remains, but now triggers the useEffect above) ---
    ...
    const handleSubmit = (e) => { 
        ...
        // Optional: Clear storage upon successful submission to prevent auto-filling a new form
    ...
    const handleSendInvitation = (e) => {
        ...
        // Optional: Clear storage after sending the invitation
        // sessionStorage.removeItem(STORAGE_KEY); 
    };
    
    const handleCancel = () => {
        console.log("Form Cancelled");
        // Optional: Clear storage on cancel
        // sessionStorage.removeItem(STORAGE_KEY);
    }
