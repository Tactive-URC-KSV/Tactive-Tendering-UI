...
<div className="p-4 mt-5 bg-white rounded-3" style={{ border: '1px solid #e0e0e0', boxShadow: '0 2px 4px rgba(0,0,0,.05)' }}>
    ...
    <div className="table-responsive ms-1 me-1">
       ...
       // --- Assuming the AttachmentRow component is being replaced or heavily modified ---
// For demonstration, let's use a simpler structure that mimics the image layout:
...
<div className="p-4 mt-5 bg-white rounded-3" style={{ border: '1px solid #e0e0e0', boxShadow: '0 2px 4px rgba(0,0,0,.05)' }}>
    ...
    <div className="ms-1 me-1 p-3" style={{ border: '1px solid #eee', borderRadius: '8px' }}>
        {Object.keys(attachments).map((key) => {
            ...
            // Loop through files if present, or just show the main category entry
            return (
                <div key={key} className="d-flex justify-content-between align-items-center py-3 border-bottom" style={{ borderColor: '#eee' }}>
                    <div className="text-start">
                        {/* Title (e.g., Technical Specifications) */}
                        <span className="fw-medium d-block mb-1" style={{ color: '#333' }}>{title}</span>
                        
                        {/* File Type/Size (e.g., pdf | 4.2 MB) - Dynamically displaying file details */}
                        {files.length > 0 && (
                            <span className="text-danger fw-medium me-2" style={{ fontSize: '13px' }}>
                                {/* Display first file type and size for simplicity, or handle multiple files */}
                                {files[0].type || files[0].extension || 'File'} | {files[0].size || 'N/A'}
                            </span>
                        )}
                        
                        {/* Description/Notes */}
                        <span className="text-muted d-block mt-1" style={{ fontSize: '13px' }}>
                            {notes}
                        </span>
                    </div>
                    
                    {/* Download Icon (Right side) - Only show if files are present */}
                    {files.length > 0 && (
                        <span 
                            className="p-2 rounded-circle" 
                            style={{ cursor: 'pointer', color: '#005197', backgroundColor: '#F0F8FF' }}
                            // onClick={() => handleDownload(files[0])} // Add your actual download handler here
                        >
                            {/* Assuming 'Download' icon is available */}
                            <Download size={18} />
                        </span>
                    )}
                </div>
        ...
        {/* If you need the horizontal rule removed from the last item, you'll need to use array indexing 
            or conditional classes, but the current structure keeps it simple. */}
    </div>
</div>
