...
// Helper component for rendering the Grid View
const GridView = () => (
            ...
            <div className="col-lg-4 col-md-6" key={index}>
                <div className="card h-100 shadow-sm" style={{ borderLeft: '5px solid #005197' }}>
                    <div className="card-body">
                        {/* ... Card Content (Resource Name, Checkbox, Details) ... */}
...
// Helper component for rendering the "Resource Working Details" Table View
const TableView = () => (
    <div className="mt-4">
        <table className="table activity-table">
            <thead>
                <tr>
                    <th style={{ width: '3%' }}>{/* Checkbox for Select All */}</th>
            ...
            <tbody>
                {estimatedResources?.map((item, index) => (
                    <tr key={index}>
                        ...
                        <td>{(item.costDetails.netQuantity).toLocaleString('en-IN', { minimumFractionDigits: 3, maximumFractionDigits: 3 })}</td>
                        <td>{(item.costDetails.costUnitRate).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                        <td>{(item.costDetails.totalCostCompanyCurrency).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                        <td>
                            <button 
                                className="btn btn-info btn-sm text-white" 
                                onClick={() => handleViewDetails(item)}
