...
<div className="ms-1">
    {Object.keys(attachments).map((key) => (
        <div key={key} className="d-flex justify-content-between py-2"
            style={{ borderBottom: "1px solid #eee" }}>
            
            <div className="text-start">
                <span className="fw-medium" style={{ color: "#333" }}>
                    {key === "technical" && "Technical Specification"}
                    {key === "drawings" && "Drawings"}
                    {key === "commercial" && "Commercial Conditions"}
                    {key === "others" && "Others"}
                </span>

                <p className="text-muted mb-0" style={{ fontSize: "13px" }}>
                    {attachments[key].files.length > 0 
                        ? `${attachments[key].files.length} file(s) attached.` 
                        : "No files attached."
                    }
                    {attachments[key].notes 
...
<div className="ms-1 me-1 p-3" style={{ border: '1px solid #eee', borderRadius: '8px' }}>
    ...
    const renderContent = () => {
        switch (currentTab) {
            ...
            case 'review':
                // inside reviewTender(), before return(...)
const formatDate = (d) => {
  if (!d) return "Not provided";
  // d may be a Date object (Flatpickr) or a string; try to normalize
  const dateObj = d instanceof Date ? d : new Date(d);
  if (isNaN(dateObj.getTime())) return "Not provided";
  return dateObj.toLocaleDateString(); // e.g. "11/22/2025" or use other options if you want
};
