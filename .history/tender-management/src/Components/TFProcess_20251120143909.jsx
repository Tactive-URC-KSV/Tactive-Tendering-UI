...
function TFProcess({ projectId }) {
    ...
    // Scope of Packages select
const scopes = useScope() || []; // returns array from Context
const scopeOptions = scopes.map(s => ({ value: s.id, label: s.scope }));
const [selectedScopes, setSelectedScopes] = useState([]); // array of selected scope ids
// When selectedScopes changes, update tenderDetail.scopeOfPackage (store array of ids)
useEffect(() => {
  setTenderDetail(prev => ({ ...prev, scopeOfPackage: Array.isArray(selectedScopes) ? selectedScopes : [] }));
}, [selectedScopes]);

// If tenderDetail gets initialized from server and contains scopeOfPackage, prefill selectedScopes
useEffect(() => {
