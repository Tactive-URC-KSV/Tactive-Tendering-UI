...
function AddResource() {
    ...
    const [selectedResource, setSelectedResource] = useState(null); // State to hold the selected resource object
    ...
    const resourceOption = useMemo(() => resources.map(item => {
        // --- DEBUG LOG: Check incoming resource data ---
        console.log(`Resource: ${item.resourceName}, Rate: ${item.rate}, UOM ID: ${item.uomId}`);
        // ----------------------------------------------
        return { 
            value: item.id, 
    ...
    // ⭐ CRITICAL EFFECT HOOK: Automatically update Rate and UOM when a Resource is selected ⭐
    useEffect(() => {
        if (selectedResource) {
            const newRate = selectedResource.defaultRate || 0;
            const newUomId = selectedResource.defaultUomId || '';
            
            // --- DEBUG LOG: Check values being used for update ---
            console.log("--- RESOURCE SELECTION TRIGGERED ---");
            console.log("Selected Resource:", selectedResource.label);
            console.log("Default Rate being set:", newRate);
            console.log("Default UOM ID being set:", newUomId);
            console.log("Total available UOM Options:", uomOptions.length);
            // ----------------------------------------------------

            // 1. Update resourceId, Rate, and UOM ID in resourceData and recalculate costs
            handleCalculations({ 
                resourceId: selectedResource.value, 
                rate: newRate,
                uomId: newUomId // Update UOM ID
            });

            // 2. Find and set the selectedUom state for the UOM Select component display
            const defaultUomOption = uomOptions.find(uom => uom.value === newUomId);
            setSelectedUom(defaultUomOption || null);
            
            // --- DEBUG LOG: Check if UOM option was found ---
            console.log("Found UOM Option:", defaultUomOption ? defaultUomOption.label : "NOT FOUND");
            console.log("---------------------------------------");
        } else {
    ...
    }, [selectedResource, uomOptions, handleCalculations]); // Triggered when selectedResource changes or UOM options are loaded
    // ⭐ END CRITICAL EFFECT HOOK ⭐
                        ...
                        <div style={{ width: '80%' }}>
                            <Select
                                ...
                                onChange={(selected) => {
                                    ...
                                    // The useEffect hook above handles the Rate and UOM defaulting
                                }}
                            />
                        </div>
                    </div>
                    ...
                    <div className="col-md-6">
                        <div style={{ width: '80%' }} className="ms-auto">
                            ...
                            <input
                                ...
                                // The value is now controlled by resourceData.rate which is set by the useEffect
                            />
                        </div>
                    </div>
                </div>
