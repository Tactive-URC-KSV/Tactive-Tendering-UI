...
function AddResource() {
  ...
  // 1) update the selectedResource UI state
  setSelectedResource(selectedOption);

  // 2) find the full resource object from `resources` (state you already fill from API)
  const selectedResObj = resources.find((r) => r.id === selectedOption?.value);

  if (selectedResObj) {
    // 3) set resourceData rate & uomId (so existing calculations/readOnly inputs pick it up)
    setResourceData((prev) => ({
      ...prev,
