 ...
 * Handles undefined values gracefully to maintain layout spacing.
 */
const DetailItem = ({ label, value }) => {
    ...
    const displayValue = isValuePresent ? value : '\u00A0'; // Non-breaking space for empty fields to hold height
        ...
        <div className="col-lg-4 col-md-6 col-sm-12 mb-3">
            ...
            <p
                ...
                style={{
                    ...
                    minHeight: '1.1em', // Maintain height for empty items
                    marginBottom: '0.5rem'
                }}
            >
                {displayValue}
...
function ContractorReview() {
    ...
    // Default to an empty object if state or formData is missing
    ...
    const handleSubmitFinal = () => {
        // Clear session data upon final submission and navigate to a confirmation/landing page
     ...
     * Assumes the file object contains 'fileData' (a data URL or server path).
     */
    const handleViewFile = (file) => {
        if (file.fileData) {
            // Opens the data URL or Blob URL/server URL in a new tab
            window.open(file.fileData, '_blank');
        } else {
            // Display an alert if the required data is missing
            alert(`Could not view "${file.name || 'document'}". File data reference is missing or invalid.`);
            
            // For a production app, this would typically involve fetching the file 
            // from a secure server endpoint using the file's ID.
        }
    };
