...
const ManualEntryForm = ({ formData, setFormData, handleChange, handleSelectChange, handleFileChange, handleRemoveFile }) => (
    <>
        ...
        {/* --- REVISED FILE UPLOAD SECTION TO MATCH REFERENCE IMAGE 1 LAYOUT --- */}
        <div className="row mb-4">
            <div className="col-12 mt-3">
                ...
                >
                    {/* Upload instructions, Icon, and Button */}
                    <UploadCloud size={30} className="mb-2" style={{ color: bluePrimary }} /> 
                    <p className="mb-1 fw-bold" style={{ color: bluePrimary }}>Optional Documents</p>
                    ...
                    {/* "Choose File" Button (Styled as a button but uses a label for the input) */}
                    <label 
                        htmlFor="fileUpload" 
                        className="btn btn-primary d-inline-flex align-items-center fw-bold px-4 mb-4"
                        style={{ 
                            backgroundColor: bluePrimary, 
                            color: "white", 
                            borderRadius: "6px",
                    ...
                    {formData.attachments && formData.attachments.length > 0 && (
                        <div className="d-flex flex-wrap justify-content-start gap-3 mt-3 pt-3 border-top w-100">
                            {formData.attachments.map((file, index) => (
                                ...
                                >
                                    ...
                                    {/* Delete Button (Default 'X' icon) */}
                                    <button 
                                        type="button" 
                                        className="btn-close ms-auto" 
                                        aria-label="Remove file" 
                                        onClick={() => handleRemoveFile(file.name)}
                                        // Using default Bootstrap style for 'X'
                                    ></button>
                                </div>
                            ))}
                        </div>
        ...
        {/* --- END REVISED FILE UPLOAD SECTION --- */}
    </>
);


// ... (AddressDetailsContent, ContactDetailsContent, TaxDetailsContent, BankAccountsContent, AdditionalInfoContent components remain the same) ...
