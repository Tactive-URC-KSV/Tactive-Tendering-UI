...
<div className="mt-5 mb-5">
    ...
    // We only care about the date part (midnight) for counting days.
    const start = new Date(startDateStr);
    const end = new Date(endDateStr);

    // Set the time of both dates to midnight (00:00:00) to ensure accurate day calculation
    // This prevents time zones or time-of-day from skewing the result.
    ...
    // Convert milliseconds to days: 1000ms/s * 60s/min * 60min/hr * 24hr/day
    const dayDiff = Math.ceil(timeDiff / (1000 * 60 * 60 * 24));

    // Ensure the result is not negative (submission date should be after floating date)
    return dayDiff > 0 ? dayDiff : 0;
};

// --- START of the code to replace inside reviewTender() ---

// 1. Calculate the difference in days using the floatingDate and submissionLastDate
const submissionDays = getDaysDifference(tenderDetail.floatingDate, tenderDetail.submissionLastDate);

// 2. Determine the text to display based on the calculation
const daysDisplay = submissionDays === null
    ? 'Not Set' // If either date is missing
    : submissionDays > 0 
        ? `${submissionDays} Day${submissionDays > 1 ? 's' : ''}` // e.g., "15 Days"
        : 'Invalid Dates'; // If submission is before or same as floating date

// Replacement JSX block for the Submission Deadline/Window in the Tender Summary
/* This entire block replaces the previous one for the Submission Deadline */
<div className="d-flex">
    <div 
        className="d-flex justify-content-between pe-2 me-4" 
        style={{ 
...
// --- END of the code to replace inside reviewTender() ---
            </div>
        </div>
    </div>
