...
const [attachments, setAttachments] = useState({
    ...
    drawings: { files: [{ /* file object */ }], notes: 'See attached drawings' },
...
const handleNoteChange = (key, value) => {
    ...
    // --- Contractor Placeholder (Replace with your actual state/API data) ---
// This list is necessary for the UI to display selected contractors as per the design.
// If you have a state variable for this, use it instead (e.g., const contractorsList = selectedContractorsList;)
const mockContractors = [ 
    { name: 'Build Tech Solutions', contact: 'John Smith | john@buildtech.com' },
    { name: 'Electro Tech Inc', contact: 'Sarah Johnson | sarah.johnson@electrotech.com' }
];

// --- Helper Component for Attachment Rows ---
// This component renders each attachment section title and status.
// This is the AttachmentRow component definition (Step 1 of the last answer)
    ...
    <div className="d-flex justify-content-between py-2" style={{ borderBottom: '1px solid #eee' }}>
        <div className="text-start">
            ...
            <p className="text-muted mb-0" style={{ fontSize: '13px' }}>
                {/* This is the line that determines if files are displayed */}
                {files.length > 0 ? `${files.length} file(s) attached.` : 'No files attached.'}
                {notes && ` (Notes: ${notes})`}
            </p>
        </div>
        {/* ... (omitted download icon section) ... */}
    </div>
);

  // =================================================================
// 1. ATTACHMENT ROW HELPER COMPONENT (MUST BE DEFINED OUTSIDE reviewTender)
// =================================================================
const AttachmentRow = ({ title, files, notes }) => (
    // Styling matches the subtle lines and text display from the Figma image
    <div className="d-flex justify-content-between py-2 align-items-center" style={{ borderBottom: '1px solid #eee' }}>
        <div className="text-start">
            <span className="fw-medium" style={{ color: '#333', fontSize: '15px' }}>{title}</span>
            <p className="text-muted mb-0" style={{ fontSize: '13px' }}>
                {/* Display file count or "No files attached." */}
                {files.length > 0 ? `${files.length} file(s) attached.` : 'No files attached.'}
                {/* Display notes if present, using the Figma structure "(Notes: ...)" */}
                {notes && notes.length > 0 && <span> (Notes: {notes})</span>} 
            </p>
        </div>
        <div>
            {/* Placeholder for the download icon (e.g., FaDownload, not included here) */}
            <span style={{ color: '#005197', cursor: 'pointer', opacity: files.length > 0 ? 1 : 0.4 }}>
                {/* Icon should be here if available */}
            </span>
        </div>
    </div>
);
...
// =================================================================
// 2. UPDATED reviewTender FUNCTION (WITH MOCK DATA INCLUDED)
// =================================================================
const reviewTender = () => {

    // -----------------------------------------------------------------
    // ⚠️ CRITICAL CHANGE: MOCK DATA DEFINITION (FOR VISUAL MATCH)
    // -----------------------------------------------------------------
    
    // --- Mock Data for General Details & Contractors ---
    const project = { projectName: 'Green Heights', projectCode: '2025' }; 
    const tenderDetail = {
        // Populated fields from Tender Details tab
        offerSubmissionMode: 'Online',
        offerSubmissionDate: 'November 30, 2025', // Renders red
        bidOpeningDate: 'December 01, 2025',
        contactPerson: 'Sarah Mitchell',
        contactMobile: '+(555) 123-4567',
        contactEmail: 'sarah.mitchell@greenheights.com',
        scopeOfWork: 'Complete construction of 5-story commercial complex including foundation, structure, MEP systems, and finishing works',
        selectedScopes: ['Civil Works', 'Electrical Systems', 'HVAC Installation', 'Safety Equipment'],
        // Populated Contractors (to override mockContractors)
        selectedContractors: [ 
            { name: 'Build Tech Solutions', contact: 'John Smith | john@buildtech.com' },
            { name: 'Electro Tech Inc', contact: 'Sarah Johnson | sarah.johnson@electrotech.com' }
        ]
    ...
    // Note: The file size in the screenshot is slightly different from the previous prompt,
    // so simplified notes are used here to match the density of the Figma image.
    const attachments = {
        technical: { files: [{name: 'tech_spec.pdf', size: 4.2}], notes: 'Detailed technical requirements...' },
        drawings: { files: [{name: 'drawing_set.png', size: 1.7}], notes: 'Complete set of architectural and engineering drawings including structural, electrical, and HVAC layouts.' },
        commercial: { files: [{name: 'commercial_conditions.xlsx', size: 3.2}], notes: 'Payment terms, penalties, insurance requirements, and other commercial conditions for the tender.' },
        others: { files: [{name: 'safety_protocols.pdf', size: 4.2}], notes: 'Safety protocols and environmental compliance requirements' },
    };
    
    // --- Mock Data for Package Details (BOQ) ---
    // This overrides the result of getSelectedLeafBoqs(parentTree) to ensure non-zero quantities
    const boqArray = [
        { id: 1, boqCode: 'a', boqName: 'Upto 1.5m depth', uom: {uomCode: 'Cum'}, quantity: 3922.4394 },
        { id: 2, boqCode: 'b', boqName: 'From 1.5m to 3m depth', uom: {uomCode: 'Cum'}, quantity: 2947.8703 },
        { id: 3, boqCode: 'c', boqName: 'From 3m to 4.5m depth', uom: {uomCode: 'Cum'}, quantity: 588.44395 },
        { id: 4, boqCode: 'd', boqName: 'From 4.5m to 6.0m depth', uom: {uomCode: 'Cum'}, quantity: 1200.0000 },
    ];
    
    // -----------------------------------------------------------------
    // ⚠️ END OF CRITICAL CHANGE. REST OF THE CODE REMAINS AS PROVIDED.
    // -----------------------------------------------------------------

    // --- 1. Get Real State Data and Prepare Helpers ---
    // The previous line 'const boqArray = getSelectedLeafBoqs(parentTree);' is now overridden.
    // The previous line 'const contractorsList = mockContractors;' is now overridden.
    
    const contractorsList = tenderDetail.selectedContractors; // Use the mock list above
    ...
    const boqNameDisplay = (boqName) => {
        return boqName && boqName.length > 30 // Adjusted length to fit the table width better
            ? boqName.substring(0, 30) + '...'
            : boqName;
    }
    
    // Total number of attachment types that have files OR notes (for the summary bar)
    const activeAttachmentCount = Object.keys(attachments).filter(key => 
        attachments[key].files?.length > 0 || attachments[key].notes
    ).length;

    // --- 2. Render Component using State Variables ---
    return (
        // The wrapper style from the Figma-matching code (light background, border)
        <div className="p-4 bg-white" style={{ borderRadius: '8px', border: '1px solid #ccc' }}> 
            
            {/* Review & Float Tender Header */}
            ...
            {/* Floating Details */}
            <div className="d-flex justify-content-start gap-5 ms-1" style={{ borderBottom: '1px solid #00519933', paddingBottom: '15px' }}>
                <div>
                    <span className="text-muted d-block" style={{ fontSize: '14px' }}>Tender Floating No</span>
                    <span className="fw-bold" style={{ color: '#005197', fontSize: '16px' }}>TF - {project?.projectCode || 'XXXX'} - 001</span> 
                </div>
                <div>
            ...
            <div className="pt-4">
                ...
                <div className="row g-4 ms-1" style={{ fontSize: '15px' }}>
                    <div className="col-md-4 text-start">
                        <span className="text-muted d-block" style={{ fontSize: '14px' }}>Project Name</span>
                        <span className="fw-medium">{project?.projectName || 'N/A'}</span>
                    </div>
                    <div className="col-md-4 text-start">
                        <span className="text-muted d-block" style={{ fontSize: '14px' }}>Offer Submission Mode</span>
                        <span className="fw-medium">{getDisplayMode(tenderDetail.offerSubmissionMode)}</span>
                    </div>
                    <div className="col-md-4 text-start">
                        <span className="text-muted d-block" style={{ fontSize: '14px' }}>Submission Last Date</span>
                        <span className="fw-medium" style={{color: tenderDetail.offerSubmissionDate ? '#dc3545' : '#333'}}>
                            {tenderDetail.offerSubmissionDate || 'N/A'}
                        </span>
                    </div>
                    <div className="col-md-4 text-start">
                        <span className="text-muted d-block" style={{ fontSize: '14px' }}>Bid Opening</span>
                        <span className="fw-medium">{tenderDetail.bidOpeningDate || 'N/A'}</span>
                    </div>
                    <div className="col-md-4 text-start">
                        <span className="text-muted d-block" style={{ fontSize: '14px' }}>Contact Person</span>
                        <span className="fw-medium">{tenderDetail.contactPerson || 'N/A'}</span>
                    </div>
                    <div className="col-md-4 text-start">
                        <span className="text-muted d-block" style={{ fontSize: '14px' }}>Contact Number</span>
                        <span className="fw-medium">{tenderDetail.contactMobile || 'N/A'}</span>
                    </div>
                    <div className="col-md-4 text-start">
                        <span className="text-muted d-block" style={{ fontSize: '14px' }}>Contact Email ID</span>
                        <span className="fw-medium">{tenderDetail.contactEmail || 'N/A'}</span>
                    </div>
                    <div className="col-md-8 text-start">
                        <span className="text-muted d-block" style={{ fontSize: '14px' }}>Scope of Work</span>
                        <span className="fw-medium">{tenderDetail.scopeOfWork || 'N/A'}</span>
                    </div>
                    <div className="col-md-12 text-start mt-4">
                        <span className="text-muted d-block" style={{ fontSize: '14px' }}>Scope of Packages</span>
                        <div className="d-flex flex-wrap gap-2 mt-1">
                            {tenderDetail.selectedScopes?.length > 0 ? (
                                tenderDetail.selectedScopes.map((scope, index) => (
                                    <span 
                                        key={index} 
                                        className="badge p-2" 
                                        style={{ backgroundColor: '#E0F0E0', color: '#2BA95A', fontSize: '12px', fontWeight: 'normal' }}
                                    >
                                        {scope}
                                    </span>
                                ))
                ...
                <hr className="mt-5" style={{ borderColor: '#00519933' }}/>
                ...
                <div className="table-responsive ms-1 me-1">
                    <table className="table table-borderless table-striped">
                        <thead style={{ color: '#005197', borderBottom: '2px solid #005197' }}>
                            <tr>
                                <th className="fw-bold" style={{ width: '20%' }}>BOQ Code</th>
                                <th className="fw-bold" style={{ width: '45%' }}>BOQ Name</th>
                                <th className="fw-bold" style={{ width: '15%' }}>Unit</th>
                                <th className="fw-bold text-end" style={{ width: '20%' }}>Quantity</th>
                            </tr>
                        </thead>
                        <tbody>
                            {boqArray.length > 0 ? (
                                boqArray.map((boq, index) => (
                                    <tr key={boq.id || index} style={{ fontSize: '15px' }}>
                                        <td>{boq.boqCode || '-'}</td>
                                        <td title={boq.boqName}>{boqNameDisplay(boq.boqName) || '-'}</td>
                                        <td>{boq.uom?.uomCode || '-'}</td>
                ...
                <div className="d-flex flex-wrap gap-3 ms-1">
                    {contractorsList.map((contractor, index) => (
                        <div 
                            key={index} 
                            className="p-3" 
                            style={{ border: '1px solid #ccc', borderRadius: '8px', minWidth: '300px' }}
                        >
                ...
                <div className="ms-1 me-1 p-3" style={{ border: '1px solid #eee', borderRadius: '8px' }}>
                    {Object.keys(attachments).map((key) => (
                        // This calls the AttachmentRow component defined above
                        <AttachmentRow 
                            key={key}
                ...
                {/* Tender Summary Section (Footer Bar) */}
                <div 
                    className="p-3 mt-5 mb-5 d-flex justify-content-between align-items-center" 
                    style={{ 
                        backgroundColor: '#F0F8FF', // Light Blue Background
                        border: '1px solid #B0C4DE', // Subtle Border
                        borderRadius: '8px',
                        minHeight: '60px'
                    }}
                >
                    <div className="d-flex gap-5 fw-bold" style={{ color: '#005197', fontSize: '15px' }}>
                        ...
                        <div>Attachments: <span style={{ color: '#333' }}>{activeAttachmentCount}</span></div>
                    ...
                    <div className="p-3 mt-5 mb-5 d-flex justify-content-between align-items-center" style={{ backgroundColor: '#F0F8FF', border: '1px solid #B0C4DE', borderRadius: '8px' }}>
                        <div className="d-flex gap-5 fw-bold" style={{ color: '#005197' }}>
                            <div>Total Packages: <span style={{ color: '#333' }}>{boqArray.length > 0 ? 1 : 0}</span></div> 
                            <div>Selected Contractors: <span style={{ color: '#333' }}>{contractorsList.length}</span></div>
                            {/* Counts sections with files OR notes */}
                        ...
                        <button 
                            ...
                            // You would put your final API call here to float the tender
                            // onClick={handleFloatTender}
                        >
                            Float Tender
                        </button>
