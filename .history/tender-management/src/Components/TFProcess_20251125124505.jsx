...
// Add these definitions inside your TenderForm/TenderDetails component function:

// 1. Hides the selected tags that normally appear inside the box.
const CustomMultiValueContainer = () => null;

// 2. Hides the dropdown indicator (the down arrow).
const CustomDropdownIndicator = () => null;

// 3. Hides the vertical line separator between the input and indicators.
const CustomIndicatorSeparator = () => null;

// 4. Hides the 'X' button that clears all values (the ClearIndicator).
...
<Select
    options={scopeOptions} // Assuming this array is available
    placeholder="Select Scope of Package"
    isMulti
    className="w-100"
    classNamePrefix="scope-select-tender" // Use a unique prefix if needed
    value={scopeOptions.filter(opt => tenderDetail.scopeOfPackage.includes(opt.value))}
    onChange={(option) => {
        // This setter updates the state for the new form
        setTenderDetail({ 
            ...tenderDetail, 
            scopeOfPackage: option ? option.map(o => o.value) : []
        });
    }}
    // HIDING LOGIC: Apply custom components to hide internal elements
    components={{
...
{/* 🛠️ EXTERNAL TAGS DISPLAY 🛠️ */}
<div className="mt-2 d-flex flex-wrap gap-2">
    {scopeOptions
        .filter(opt => tenderDetail.scopeOfPackage.includes(opt.value))
        .map(selectedOpt => (
            <span key={selectedOpt.value} className="select__multi-value">
                <span className="select__multi-value__label">
                    {selectedOpt.label}
                </span>
                {/* Functional External Remove 'X' Button */}
                <span
                    className="select__multi-value__remove"
                    onClick={(e) => {
                        e.preventDefault();
                        // Define this function to handle state removal in this component
                        handleRemoveTenderScope(selectedOpt.value); 
                    }}
                >
                    &times;
                </span>
