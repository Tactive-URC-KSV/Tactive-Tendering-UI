...
const ManualEntryForm = ({ formData, setFormData, handleChange, handleSelectChange, handleFileChange, handleRemoveFile }) => (
    ...
    // Add accepted types for user experience
...
const fileToBase64 = (file) => {
    return new Promise((resolve, reject) => {
        ...
        // This resolves the promise only after the file data is fully loaded
        reader.onload = () => resolve(reader.result); 
        
        // This captures any read errors
        reader.onerror = error => {
            console.error("FileReader Error during Base64 conversion:", error);
            reject(error);
        };
        
        // Starts reading the file content as a Data URL (which includes the MIME type and Base64)
...
const [formData, setFormData] = useState(loadFormData());
 ...
 // In ContractorOverview.jsx

// ... (keep the existing code above this function)

// The modified handleFileChange function:
const handleFileChange = async (event) => {
    ...
    for (const file of files) {
        try {
            ...
            // 🛑 ADDED STEP 1A: CRITICAL DATA VALIDATION 
            if (fileDataUrl.length < 100) {
                 console.error(`[Base64 Error] File: ${file.name}. Data URL looks too short. Length: ${fileDataUrl.length}`);
                 alert(`Error: Failed to read file ${file.name}. Please try a smaller file or a different format.`);
                 continue; // Skip this file
            }

            console.log(`[Base64 Success] File: ${file.name}, Base64 Length: ${fileDataUrl.length}`);

            newAttachments.push({
                name: file.name,
                size: file.size,
                lastModified: file.lastModified,
                fileData: fileDataUrl // <<< Base64 content is stored here
            });
        } catch (error) {
            console.error(`Failed to process file ${file.name}`, error);
            alert(`A critical error occurred while processing ${file.name}. See console.`);
        }
    }
    
    // Update the form state after all files have been processed
    setFormData(prevData => {
        const updatedData = {
            ...prevData,
            attachmentMetadata: [...prevData.attachmentMetadata, ...newAttachments]
        };
        
        // 🛑 ADDED STEP 1B: SESSION STORAGE CHECK
        try {
             // Check if the total data size exceeds sessionStorage limit (e.g., 5MB)
             // This is an approximate check, as string length != byte size, but it helps.
             const totalDataSize = JSON.stringify(updatedData).length;
             if (totalDataSize > 5 * 1024 * 1024) { 
                 console.warn(`[Storage Warning] Total form data size is ${totalDataSize / (1024 * 1024).toFixed(2)} MB. This might fail to save to sessionStorage.`);
             }
             sessionStorage.setItem(STORAGE_KEY, JSON.stringify(updatedData));
        } catch (storageError) {
             console.error("[CRITICAL Storage Error] Failed to save form data (likely too large):", storageError);
             alert("File upload failed. The file size exceeds the form data limit. Please use smaller files.");
        }
...
const handleSubmit = (e) => { 
    ...
    // 🛑 ADDED STEP 2: PRE-SUBMISSION CHECK
    console.log("--- FINAL SUBMISSION DATA CHECK ---");
    formData.attachmentMetadata.forEach((file, index) => {
        const status = file.fileData && file.fileData.length > 100 ? 'SUCCESS' : 'FAILURE (Data Missing)';
        console.log(`Attachment ${index + 1}: ${file.name} -> ${status}`);
    });
    console.log("-----------------------------------");

    // Only navigate if at least one crucial piece of data exists
    if (formData.entityCode || formData.attachmentMetadata.length > 0) {
