...
// import Flatpickr from "react-flatpickr"; // Flatpickr is imported but not used, kept commented out.

const bluePrimary = '#005197';

// --- Placeholder Options (To prevent errors) ---
...
const ManualEntryForm = ({ formData, setFormData, handleChange, handleSelectChange, handleFileChange, handleRemoveFile }) => (
    <>
        ...
        {/* --- CORRECTED FILE UPLOAD SECTION: Instructions first, then list --- */}
        <div className="row mb-4">
            <div className="col-12 mt-3">
                <h4 className="fs-6 fw-bold mb-3" style={{ color: bluePrimary }}>Attachment (Certificates/Licenses)</h4>
            
                {/* Upload Drop Zone and File List (Combined) */}
                <div 
                ...
                >
                    {/* File Input (Hidden, but label links to it) */}
                    <input 
                        type="file" 
                        id="fileUpload" 
                        name="attachments" 
                    ...
                    {/* 1. Upload instructions and icon (Moved to the top) */}
                    <label htmlFor="fileUpload" className="d-block cursor-pointer mb-3">
                    ...
                    {/* 2. File List Display (Moved below instructions) */}
                    {formData.attachments && formData.attachments.length > 0 && (
                        // Added border-top and padding to separate list visually
                        <div className="d-flex flex-wrap justify-content-center gap-3 mt-3 pt-3 border-top w-100"> 
                            {formData.attachments.map((file, index) => (
                                <div 
                                    key={index} 
                                    className="border rounded p-2 position-relative bg-light" 
                                    style={{ 
                                        backgroundColor: '#F7F7F7', 
                                        border: '1px solid #D1D1D1', // Added border for clarity
                                        maxWidth: '200px', 
                                        overflow: 'hidden',
                                        whiteSpace: 'nowrap',
                                    }}
                                >
                                    ...
                                    {/* Delete Button (Red 'X' - Corrected Styling) */}
                                    <button 
                                        type="button" 
                                        className="btn-close ms-auto" 
                                        aria-label="Remove file" 
                                        onClick={() => handleRemoveFile(file.name)}
                                        // Ensured red styling works by overriding Bootstrap's default button-close styles
                                        style={{ 
                                            color: 'red', 
                                            opacity: 1, 
                                            filter: 'none', 
                                            ...
                                            height: '8px', 
                                            fontSize: '8px',
                                            // Using an SVG approach to force red on the 'X' glyph might require different CSS if btn-close is stubborn, 
                                            // but your inline styles are the correct way to attempt the override.
                                            backgroundImage: "url(\"data:image/svg+xml,%3csvg viewBox='0 0 16 16' fill='%23dc3545' xmlns='http://www.w3.org/2000/svg'%3e%3cpath d='M.293.293a1 1 0 011.414 0L8 6.586 14.293.293a1 1 0 111.414 1.414L9.414 8l6.293 6.293a1 1 0 01-1.414 1.414L8 9.414l-6.293 6.293a1 1 0 01-1.414-1.414L6.586 8 .293 1.707a1 1 0 010-1.414z'/%3e%3c/svg%3e\")",
                                        }} 
                                    ></button>
                                </div>
                            ))}
        ...
        {/* --- END CORRECTED FILE UPLOAD SECTION --- */}
    </>
);
...
function ContractorOverview() {
    ...
    // Initial state setup (attachments is an empty array)
    const [formData, setFormData] = useState({
        entityCode: '', entityName: '', effectiveDate: '', entityType: '',
        natureOfBusiness: '', grade: '', 
        attachments: [], // Initialized as an array for multiple files
        phoneNo: '', emailID: '',
    ...
    // Handler to add multiple files to the attachments array
    ...
    // Handler to remove a specific file by name
