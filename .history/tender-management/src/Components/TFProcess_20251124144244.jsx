...
/**
 * Calculates the difference in full days between two date strings.
 * @param {string | Date} floatingDate The Tender Floating Date.
 * @param {string | Date} lastSubmissionDate The Submission Last Date.
 * @returns {string} e.g., "10 days", "5 days", or a status message.
 */
const calculateDaysDifference = (floatingDate, lastSubmissionDate) => {
    // Ensure both dates are present before calculation
    if (!floatingDate || !lastSubmissionDate) {
        return 'Not Set';
    }

    // Create Date objects from the input strings
    const start = new Date(floatingDate);
    const end = new Date(lastSubmissionDate);
    ...
    // 2. Convert milliseconds to full days (1000 ms * 60 s * 60 min * 24 hrs).
    // Math.ceil is used to count the last partial day as a full day.
...
<div className="mt-5 mb-5">
    ...
    >
        ...
        <div className="d-flex flex-column" style={{ color: '#005197', fontSize: '14px' }}> 
         ...
         // Assume this JSX is inside your main React component's render function:

<div className="mt-5 mb-5">
    
    {/* 1. TENDER SUMMARY BOX */}
    <div 
        className="p-4 d-flex flex-column rounded-3" 
        style={{ 
            backgroundColor: '#F7F8FC', // Very light blue/off-white background
            border: '1px solid #D2E3F4', // Soft light blue border
        }}
    >
        
        {/* Box Header: Tender Summary (Left-Aligned) */}
        <div className="mb-3 fw-bold text-start" style={{ color: '#005197' }}> 
            Tender Summary
        </div>
        
        {/* Statistics Section - Structured into two columns/rows */}
        <div className="d-flex flex-column" style={{ color: '#005197', fontSize: '14px' }}> 
            
            {/* Row 1: Total Packages and Selected Contractors */}
            <div className="d-flex mb-2"> 
                {/* Total Packages - LEFT COLUMN (Single Vertical Separator) */}
                <div 
                    className="d-flex justify-content-between pe-2 me-4" 
                    style={{ 
                        width: '50%', 
                ...
                {/* Selected Contractors - RIGHT COLUMN */}
                <div className="d-flex justify-content-between ps-4" style={{ width: '50%' }}>
                    <span>Selected Contractors:</span> <span style={{ color: '#005197' }}>{contractorsList.length}</span> 
                </div>
            </div>
            
            {/* Row 2: Attachments and Submission Deadline */}
            <div className="d-flex">
                {/* Attachments - LEFT COLUMN (Single Vertical Separator) */}
                <div 
                    className="d-flex justify-content-between pe-2 me-4" 
                    style={{ 
                        width: '50%',
                ...
                {/* Submission Deadline - RIGHT COLUMN (Calculated Days) */}
                <div className="d-flex justify-content-between ps-4" style={{ width: '50%' }}>
                    <span>Submission Deadline:</span> 
                    <span 
                        style={{ 
                            color: '#dc3545' // Red color for the deadline 
                        }}
                    >
                        {/* Ensure you use the correct variable names for your dates */}
                        {calculateDaysDifference(tenderDetail.floatingDate, tenderDetail.offerSubmissionDate)}
                    </span>
                </div>
            </div>
        </div>
    ...
    {/* 2. FLOAT TENDER BUTTON (Below the Summary Box, Aligned Right) */}
