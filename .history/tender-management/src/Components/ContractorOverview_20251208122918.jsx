...
/**
 * Email Invite Form
 * FIX: Removed unnecessary handleSelectChange prop and ensured all inputs have 'name'.
 */
...
const ContractorOverview = () => {
    ...
    const getButtonStyle = (view) => ({
        ...
        borderRadius: selectedView === view ? '6px' : '6px', // Fixed string termination
    });
                    ...
                    <div className="card text-start border-0 shadow-sm" style={{ borderRadius: "8px", padding: selectedView === 'manual' ? "0 1.5rem 1.5rem 1.5rem" : "1.5rem" }}>
                        {selectedView === 'manual' ? 
                            <ManualEntryForm 
                                ...
                                handleFileChange={handleFileChange} 
                            /> : 
                            <EmailInviteForm // Corrected props passed here
                                formData={formData} 
                                handleChange={handleChange} 
                                handleFileChange={handleFileChange} 
                            />
