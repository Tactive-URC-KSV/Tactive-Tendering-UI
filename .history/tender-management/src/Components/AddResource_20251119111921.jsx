...
function AddResource() {
    ...
    axios
      .get(`${import.meta.env.VITE_API_BASE_URL}/uom`, {  // Replace `/uom` with your actual endpoint
        headers: {
          Authorization: `Bearer ${sessionStorage.getItem('token')}`,
          'Content-Type': 'application/json',
        },
      ...
      .then((res) => {
  if (res.status === 200) {
    console.log("UOM fetched:", res.data); // <--- add this
    setUom(res.data);
  }
})
      ...
      .catch((err) => {
        if (err?.response?.status === 401) handleUnauthorized();
        else toast.error('Failed to fetch UOM.');
      });
    }, []);

    useEffect(() => { fetchUOM(); }, [fetchUOM]);
    const uomOptions = useMemo(
    () => uom.map(item => ({ value: item.id, label: item.uomName })),  // Adjust property names based on API
    [uom]
    );
            ...
            <FormSectionContainer title="Quantity & Measurements" icon={<AlignLeft size={20} className="text-primary" />} defaultOpen={true}>
                <div className="row g-3">
                    <div className="col-md-6">
                        ...
                        <div style={{ width: '80%' }}>
                            <Select options={uomOptions} styles={customStyles} placeholder="Select UOM" className="w-100"classNamePrefix="select" onChange={(selected) => setSelectedUom(selected)}/>
                        </div>
                    </div>
    ...
    <div className="row g-4">
        <div className="col-md-6">
            ...
            <div className="mb-3">
                ...
                <input type="number" className="form-control" placeholder="0.00"style={{ borderRadius: "0.5rem" }} />
            </div>
        ...
        <div className="col-md-6">
            ...
            <div className="mb-3">
                ...
                <input type="number" className="form-control" placeholder="1.00000" style={{ borderRadius: "0.5rem" }} />
             </div>
