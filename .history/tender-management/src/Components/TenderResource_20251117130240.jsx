...
<button 
            // Button style is blue (btn-primary) but with no text, exactly like the image.
            className="btn btn-primary me-2"
            // Toggle logic: If current view is 'table', switch to 'grid', otherwise switch to 'table'.
            onClick={() => setViewType(viewType === 'table' ? 'grid' : 'table')}
            title={viewType === 'table' ? 'Switch to Grid View' : 'Switch to Table View'}
        >
            {/* The icon changes based on the current viewType */}
            {viewType === 'table' ? (
                // If currently showing table, show the Grid icon to switch
                <Grid size={20} color="white" />
            ) : (
                // If currently showing grid, show the Table icon to switch
