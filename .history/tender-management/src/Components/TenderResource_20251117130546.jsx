...
<div className="btn-group me-2" role="group" aria-label="View switch">
            {/* Table View Button (Left side of the segmented control) */}
            <button
                type="button"
                className={`btn btn-sm text-center ${viewType === 'table' ? 'btn-primary' : 'btn-light'}`}
                onClick={() => setViewType('table')}
                style={{ 
                    // Set border/padding to mimic the segmented look
                    borderRadius: '5px 0 0 5px', 
                    borderRight: viewType === 'grid' ? '1px solid #005197' : 'none', 
                    padding: '8px 12px' 
                }}
                title="Table View"
            >
                {/* Assuming Table icon is the one on the left in the segmented control */}
                <Table size={20} color={viewType === 'table' ? 'white' : '#005197'} />
            </button>
            
            {/* Grid View Button (Right side of the segmented control) */}
            <button
                type="button"
                className={`btn btn-sm text-center ${viewType === 'grid' ? 'btn-primary' : 'btn-light'}`}
                onClick={() => setViewType('grid')}
                style={{ 
                    // Set border/padding to mimic the segmented look
                    borderRadius: '0 5px 5px 0', 
                    borderLeft: viewType === 'table' ? '1px solid #005197' : 'none',
                    padding: '8px 12px' 
                }}
                title="Grid View"
            >
                {/* Assuming Grid icon is the one on the right in the segmented control */}
                <Grid size={20} color={viewType === 'grid' ? 'white' : '#005197'} />
            </button>
        </div>
          </div>
        </div>
