...
import { ArrowLeft, ArrowRight, BoxesIcon, ChevronDown, ChevronRight, IndianRupee, Info, Paperclip, Plus, User2, X, CloudUpload} from 'lucide-react'; 
...
function TFProcess({ projectId }) {
  ...
  const handleDrop = (e, key) => {
    e.preventDefault();
    setFiles({ ...files, [key]: e.dataTransfer.files[0] });
  };
  ...
  const renderCard = (key, title, placeholder) => (
    <Col md={6}>
      <Card
        className="p-4 text-center border-primary"
      ...
      >
        <div>
          ...
          <p className="fw-semibold mt-3">{title}</p>
          ...
          {files[key] && (
            <p className="mt-3 text-success">Uploaded: {files[key].name}</p>
          )}
        </div>
  ...
  return (
    <div className="container mt-4 mb-5">
      ...
      <p className="text-muted">Upload required documents</p>

      <Row className="gy-4">
        {renderCard(
          "technicalSpecification",
          "Technical Specification",
          "Additional notes for technical specification"
        )}
        {renderCard("drawings", "Drawings", "Additional notes for drawings")}
        {renderCard(
          "commercialConditions",
          "Commercial Conditions",
          "Additional notes for Commercial Conditions"
        )}
        {renderCard("others", "Others", "Additional notes for others")}
      </Row>

      <div className="text-end mt-4">
        <button className="btn btn-primary px-4">Review & Confirm →</button>
      </div>
    </div>
  );
}
