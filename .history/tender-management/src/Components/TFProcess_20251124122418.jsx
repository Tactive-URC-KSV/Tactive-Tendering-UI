...
<div className="p-4 mt-5 bg-white rounded-3" style={{ border: '1px solid #e0e0e0', boxShadow: '0 2px 4px rgba(0,0,0,.05)' }}>
    ...
    <div className="table-responsive ms-1 me-1">
      ...
      {/* 5. ATTACHMENTS - SINGLE WHITE CONTAINER CARD (Matching Figma Style) */}
<div className="p-4 mt-5 bg-white rounded-3" style={{ border: '1px solid #e0e0e0', boxShadow: '0 2px 4px rgba(0,0,0,.05)' }}>
    ...
    {/* Content Area (Inner Gray Bordered Box) */}
    <div className="ms-1 me-1 p-3" style={{ border: '1px solid #eee', borderRadius: '8px' }}>
        {Object.keys(attachments).map((key) => {
            ...
            // IMPORTANT: Ensure getFriendlyFileType and formatFileSize are defined outside your component.

            return (
                <div 
                    key={key} 
                    // Adds a border below each row except potentially the last one
                    className="d-flex justify-content-between align-items-center py-3 border-bottom" 
                    style={{ borderColor: '#eee' }}
                >
                    <div className="text-start me-4 flex-grow-1">
                        
                        {/* 1. Title (e.g., Technical Specifications) - BOLD TEXT */}
                        <span className="fw-medium d-block mb-1" style={{ color: '#333' }}>{title}</span>
                        
                        {/* 2. File Type and Size (MUTED GRAY, SPACE Separated) */}
                        {fileInfo ? (
                            <div className="d-block mt-1">
                                {/* Style matches the muted text in your screenshot */}
