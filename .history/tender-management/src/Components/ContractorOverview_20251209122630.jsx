...
import { ArrowLeft, Pencil, Mail, UploadCloud, FileText, Building, PhoneCall, Receipt, Landmark, Info, X } from "lucide-react"; 
    ...
    <>
        ...
        <div className="row mb-4">
            <div className="col-12 mt-3">
                ...
                <div className="p-4 text-center rounded" style={{ cursor: "pointer", border: `2px dashed ${bluePrimary}B2`, borderRadius: "8px" }}>
                    <input
                        type="file"
                        id="fileUpload"
                        name="attachments"
                    ...
                    {formData.attachments && formData.attachments.length > 0 && (
                        <div className="d-flex flex-wrap justify-content-center gap-3 mt-3">
                            {formData.attachments.map((file, index) => (
                                <div key={index} className="d-flex align-items-center p-2 border rounded bg-light" style={{ maxWidth: "200px", overflow: "hidden", whiteSpace: "nowrap" }}>
                                    <span className="text-truncate small me-2" title={file.name}>{file.name}</span>
                                    <button type="button" className="ms-auto bg-transparent border-0" onClick={() => handleRemoveFile(file.name)}>
                                        <X color="red" size={18} />
                                    </button>
                                </div>
                            ))}
...
// [Use exactly same as in your code above, since you already included all fields with Selects and Inputs]
...
function ContractorOverview() {
    ...
    const [formData, setFormData] = useState({
        ...
        natureOfBusiness: '', grade: '', attachments: [],
    ...
    const handleRemoveFile = (fileNameToRemove) => {
        setFormData(prev => ({ ...prev, attachments: prev.attachments.filter(file => file.name !== fileNameToRemove) }));
    };

    const handleSubmit = (e) => { e.preventDefault(); console.log("Manual Form Submitted:", formData); };
    const handleSendInvitation = (e) => { e.preventDefault(); console.log("Invitation Sent:", { contractorEmailId: formData.contractorEmailId, contractorName: formData.contractorName, message: formData.contractorMessage }); };
        ...
        <div className="container-fluid min-vh-100 p-0">
            ...
            <div className="px-4 text-start">
                <div className="d-inline-flex mb-4" style={buttonGroupStyle}>
                    <button className="btn d-flex align-items-center fw-bold" onClick={() => setSelectedView('manual')} style={manualButtonStyle}>
                        <Pencil size={20} className="me-2" /> Manual Entry
                    </button>
                    <button className="btn d-flex align-items-center fw-bold" onClick={() => setSelectedView('email')} style={emailButtonStyle}>
