...
const currencyOptions = useMemo(
    ...
    // State to hold resource natures and loading state
const [resourceNatures, setResourceNatures] = useState([]);
const [resourceNaturesLoading, setResourceNaturesLoading] = useState(true);

// Fetch function using useCallback
const fetchResourceNature = useCallback(() => {
  setResourceNaturesLoading(true); // start loading
  axios
    ...
    .then((res) => {
      if (res.status === 200) {
        setResourceNatures(res.data); // set fetched data
      }
    })
    .catch((err) => {
      if (err?.response?.status === 401) {
        handleUnauthorized(); // handle unauthorized
      } else {
        toast.error('Failed to fetch resource natures.');
      }
    })
    .finally(() => setResourceNaturesLoading(false)); // stop loading
}, []);
...
const resourceNatureOptions = useMemo(
  () =>
    resourceNatures.map((item) => ({
      value: item.id,
      label: item.resourceNatureName,
    })),
  [resourceNatures]
);
  ...
  <div style={{ width: '80%' }} className="ms-auto">
    ...
    <Select
      options={
        resourceNatures.length > 0
          ? resourceNatureOptions
      ...
      isLoading={resourceNatures.length === 0}
      isDisabled={resourceNatures.length === 0}
      isClearable
    />
  </div>
</div>
            ...
            <FormSectionContainer title="Wastage & Net Quantity" icon={<Settings size={20} className="text-primary" />}><p className="text-muted">Fields for Wastage and Net Quantity will go here...</p></FormSectionContainer><FormSectionContainer title="Pricing & Currency" icon={<DollarSign size={20} className="text-primary" />}>
    <div style={{ width: '80%' }}>
        <label className="form-label text-start w-100">
