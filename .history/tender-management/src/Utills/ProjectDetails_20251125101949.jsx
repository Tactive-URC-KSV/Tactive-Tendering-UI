...
export default function ScopeSelect({ scopeOptions, scopePack, setScopePack }) {
  return (
    <div className="row align-items-center ms-4 me-4">
      <div className="col-12 mt-3">
        ...
        {/* Display selected items outside the select */}
        <div className="selected-items-container mb-2">
          {scopePack.map((val) => {
            const item = scopeOptions.find((opt) => opt.value === val);
            return (
              <span key={val} className="selected-item-chip">
                {item?.label}
              </span>
            );
          })}
        ...
        <Select
          ...
          components={{
            MultiValue: () => null, // hide default multi-value chips
