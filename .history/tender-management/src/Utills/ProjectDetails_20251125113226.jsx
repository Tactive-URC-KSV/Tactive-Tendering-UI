...
function ProjectInfo({ project, handleSubmit, region, scopePack, sector, setProject, setRegion, setSector, setScopePack, setUom, uom, loading, fileInputRef, uploadedFiles, setUploadedFiles }) {
    ...
    import { components } from 'react-select'; // <-- Add this import!
// ... (Your existing code)

// 1. Custom component to RENDER the tags OUTSIDE the main control
const CustomValueContainer = ({ children, ...props }) => {
    // Separate the input/placeholder (index 0) from the selected values (index 1)
    const [inputValue, selectedValues] = children;
    
    // We only render the input/placeholder inside the control
    return (
...
// 2. Custom component to handle how the tags are displayed BELOW the control
const MultiValueLabel = ({ children, data, selectProps }) => {
    return (
        <div 
            className="scope-tag-wrapper" // <-- We will style this new wrapper in CSS
            key={data.value}
        >
            ...
            <FaTimes 
                ...
                onClick={() => selectProps.onRemoveValue(data)}
            />
        </div>
    );
};
    ...
    return (
        <div className="project-info-input">
            ...
            <div className="mt-3 mb-4 pb-5 bg-white rounded-3" style={{ border: '0.5px solid #0051973D' }}>
               ...
               // ... (around line 200 in your component)
...
                            components={{
                                MultiValue: MultiValueLabel, // Use our custom component for the tags
                                ValueContainer: CustomValueContainer, // Use our custom container
                                MultiValueRemove: () => null, // Hide the default 'X' on the tag
                            }}
                            value={scopeOptions.filter(opt => scopePack.includes(opt.value))}
                            onRemoveValue={(removedOption) => setScopePack(prev => prev.filter(id => id !== removedOption.value))} // New handler for custom 'X'
                            onChange={(option) => setScopePack(option ? option.map(o => o.value) : [])}
                        />
                        
                        {/* 🚨 NEW: RENDER SELECTED TAGS HERE, OUTSIDE THE CONTROL */}
                        <div className="d-flex flex-wrap mt-2">
                            {scopeOptions.filter(opt => scopePack.includes(opt.value)).map(tag => (
                                <div key={tag.value} className="scope-tag-wrapper">
                                    <span className="scope-tag-label">{tag.label}</span>
                                    <FaTimes
                                        className="scope-tag-remove"
                                        size={12}
                                        onClick={() => setScopePack(prev => prev.filter(id => id !== tag.value))}
                                    />
                                </div>
                            ))}
                        </div>
