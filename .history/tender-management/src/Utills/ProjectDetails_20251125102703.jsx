...
function ProjectInfo({ project, handleSubmit, region, scopePack, sector, setProject, setRegion, setSector, setScopePack, setUom, uom, loading, fileInputRef, uploadedFiles, setUploadedFiles }) {
  ...
  hideSelectedOptions={false}  // selected options remain in dropdown
/>
<div className="selected-options-container mt-2">
  {scopeOptions
    .filter(opt => scopePack.includes(opt.value))
    .map((opt) => (
      <div key={opt.value} className="selected-option">
        <span>{opt.label}</span>
        <span
          className="remove-x"
          onClick={() =>
            setScopePack(prev => prev.filter(val => val !== opt.value))
          }
        >
          ×
        </span>
